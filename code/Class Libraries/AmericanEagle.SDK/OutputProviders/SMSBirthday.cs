...
namespace AmericanEagle.SDK.OutputProviders
{
    ...
    /// Class SMSBirthday
    /// </summary>
    public class SMSBirthday : IDAPOutputProvider
    {

        private string className = MethodBase.GetCurrentMethod().DeclaringType.Name;

        private string outfilename = "AEORW_SM_Brthday_YYYYMMDDHHMMSS.txt";
        ...
        public void ProcessMessageBatch(IList<string> messageBatch)
        {
            ...
            try
            {
                ...
                // Get XML Node
                XmlNode xmlNode = doc.SelectSingleNode("CheetahMobileBirthday/Global");
                    ...
                    if (!string.IsNullOrEmpty(strLoyaltyNumber))
                    {
                        ...
                        if (memberDetails != null)
                        {
                            ...
                            if (Utilities.isInPilot(memberDetails.ExtendedPlayCode))   // AEO-point conversion
                            {
                                ...
                                string offerTypeCode = utils.GetOffer(birthdayTypeCode, DateTime.Today.AddMonths(1).Month, FileUtils.OfferType.SMS);
