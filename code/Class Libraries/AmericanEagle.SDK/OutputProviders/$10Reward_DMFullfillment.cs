...
{
    public class Reward10_DMFullfillment : IDAPOutputProvider
    {
        private LWLogger logger = LWLoggerManager.GetLogger(LWConstants.LW_DAP_SERVICE);

        private string directFileName = "AEORW_DM_10RWARD_YYYYMMDDHHMMSS.txt";
        ...
        public void Initialize (  NameValueCollection globals, NameValueCollection args, long jobId, 
                                  DAPDirectives config, NameValueCollection parameters, 
                                  DAPPerformanceCounterUtil performUtil )
        {
            ...
            directFileName = path+ directFileName.Replace("YYYYMMDDHHMMSS", 
                                                            this.processDate.ToString("yyyyMMddHHmmss"));
            ...
            try
            {
                ...
                if ( xmlNode != null )
                {
                    if ( ipCode > 0 )
                    {
                        ...
                        Member member = LWDataServiceUtil.DataServiceInstance(true).LoadMemberFromIPCode(ipCode);
                        if ( member == null )
                        {
                        ...
                        else
                        {
                            ...
                            foreach (long memberRewardId in memberRewardIDs)
                            {
                                ...
                                this.logger.Trace(MethodBase.GetCurrentMethod().DeclaringType.Name, 
                                    MethodBase.GetCurrentMethod().Name, 
                                    "Member Reward = " + memberReward.Id);

                                this.logger.Trace(MethodBase.GetCurrentMethod().DeclaringType.Name, 
                                    MethodBase.GetCurrentMethod().Name, 
                                    "Member Reward is " + memberReward == null ? "null" : "not null");
                                ...
                                if ( lCard == null )
                                {
                                    throw new Exception("It was not possible to determine the " + 
                                        "loyaltynumber because no primary card was found for ipcode=" +
                                        member.IpCode);
                                ...
                                fileLine.TIER_STATUS = tierTmp == null || tierTmp.TierDef == null ? 
                                    string.Empty : 
                                    tierTmp.TierDef.Name;
                                ...
                                fileLine.BIRTHDATE = member.BirthDate == null? 
                                    "" :
                                    String.Format("{0:dd-MMM-yyyy}", member.BirthDate);

                                                              
                                IList<IClientDataObject> loDetails = member.GetChildAttributeSets("MemberDetails");

                                MemberDetails details = (loDetails == null || loDetails.Count == 0 ? 
                                    null : 
                                    loDetails[0]) as MemberDetails;
                                ...
                                {
                                    if ( details.PendingCellVerification == 1 && 
                                        details.PendingEmailVerification == 1 )
                                    {
                                        fileLine.AAP_FLAG = "4";
                                    }
                                }
                                ...
                                string tmpEID = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("$10_EID_EmailReward").Value;
                                string tmpCampaignID = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("$10_CampaignID_SMSReward").Value;
                                ...
                                RewardDef def10 = LWDataServiceUtil.DataServiceInstance(true).GetRewardDef("AEO Rewards $10 Reward");
                                ...
                                IList<MemberReward> rewards10 =LWDataServiceUtil.DataServiceInstance(true).GetMemberRewardsByDefId(member, def10.Id);
        ...
        public void ExecuteRule ( Member member, out List<long> memberRewardIds)
        {
            try
            {
                ...
                additionalFields.Add("TypeCode", "5DDM"); //keep using 5DDM
