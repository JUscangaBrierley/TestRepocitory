...
{
    class B5G1Fulfillment : IDAPOutputProvider
    {
        ...
        private string mobileFileName = "AEORW_SM_B5G1_RW_YYYYMMDDHHMMSS.txt";
        private string emailFileName = "AEORW_EM_B5G1_RW_YYYYMMDDHHMMSS.txt";

        // AEO-Redesign-2015 End

        private long rowsWrittenMobile = 0;
        ...
        public void Initialize(System.Collections.Specialized.NameValueCollection globals, System.Collections.Specialized.NameValueCollection args, long jobId, DAPDirectives config, System.Collections.Specialized.NameValueCollection parameters, DAPPerformanceCounterUtil performUtil)
        {
            ...
            mobileFileName = path + mobileFileName.Replace("YYYYMMDDHHMMSS", DateTime.Now.ToString("yyyyMMddHHmmss"));
            emailFileName = path + emailFileName.Replace("YYYYMMDDHHMMSS", DateTime.Now.ToString("yyyyMMddHHmmss"));

            // we allways create the  2 possible output files
            //

            string[] files = { mobileFileName, emailFileName };
            ...
            try
            {
                ...
                if (null != xmlNode)
                {
                    if (ipcode > 0)
                    {
                        ...
                        {
                            member = lwService.LoadMemberFromIPCode(ipcode); //AEO-Redesegin-2015 Begin & End
                        }
                                ...
                                foreach (string ruleResult in rewardsIds)
                                {
                                    ...
                                    if (long.TryParse(tmpId, out memberRewardId))
                                    {
                                        ...
                                        if (memberReward != null)
                                        {
                                            ...
                                            {
                                                tmpEID = (rowType == "Jean") ?
                                                        dtService.GetClientConfiguration("EID_EmailJeanReward").Value
                                                        : dtService.GetClientConfiguration("EID_EmailBraReward").Value;
                                                //AEO-Redesign-2015 End


                                                tmpCampaignID = (rowType == "Jean") ?
                                                       dtService.GetClientConfiguration("CampaignID_SMSJeanReward").Value
                                                       : dtService.GetClientConfiguration("CampaignID_SMSBraReward").Value;
                                            }
                                            ...
                                            using (var lwService = _dataUtil.LoyaltyDataServiceInstance())
                                            {
                                                ...
                                                if (loDetails != null && loDetails.Count > 0)
                                                {
                                                    ...
                                                    if (loTmp.PendingCellVerification == 0) // AEO-557 begin & end
                                                    {
                                                    ...
                                                    //if ( loTmp.PendingEmailVerification == 0 ) // AEO-557 begin & end
