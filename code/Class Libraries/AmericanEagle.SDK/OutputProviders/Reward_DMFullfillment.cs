...
{
    public class Reward_DMFullfillment : IDAPOutputProvider
    {
        private LWLogger logger = LWLoggerManager.GetLogger(LWConstants.LW_DAP_SERVICE);

        private string directFileName = "AEORW_DM_5Reward_YYYYMMDDHHMMSS.txt";
        ...
        public void Initialize ( System.Collections.Specialized.NameValueCollection globals, System.Collections.Specialized.NameValueCollection args, long jobId, DAPDirectives config, System.Collections.Specialized.NameValueCollection parameters, DAPPerformanceCounterUtil performUtil )
        {
            ...
            directFileName = path+ directFileName.Replace("YYYYMMDDHHMMSS", this.processDate.ToString("yyyyMMddHHmmss"));
                    ...
                    if ( ipCode > 0 )
                    {
                        ...
                        else
                        {
                            ...
                            foreach (long memberRewardId in memberRewardIDs)
                            {
                                ...
                                string tmpEID = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("EID_EmailReward").Value;
                                string tmpCampaignID = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("CampaignID_SMSReward").Value;
        ...
        public void ExecuteRule ( Member member, out List<long> memberRewardIds)
        {
            try
            {
                ...
                additionalFields.Add("TypeCode", "5DDM");
