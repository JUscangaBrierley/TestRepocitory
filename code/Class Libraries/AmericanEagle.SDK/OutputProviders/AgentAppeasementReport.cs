...
{
    class AgentAppeasementReport : IDAPOutputProvider
    {
        ...
        private long LastAgentId = -1;
        private long lineCount = 0;
        private bool printNewAgent = true;
        


        private string outputFileName = "AgentAppeasementReport_MMM_YYYY.txt";
        ...
        public void Initialize(System.Collections.Specialized.NameValueCollection globals, System.Collections.Specialized.NameValueCollection args, long jobId, DAPDirectives config, System.Collections.Specialized.NameValueCollection parameters, DAPPerformanceCounterUtil performUtil)
        {
            ...
            foreach ( string filestr in files )
            {
                ...
                sw.Write(this.getHeaderline(this.processDate));
                sw.Flush();
        ...
        public void ProcessMessageBatch(List<string> messageBatch)
        {
            ...
            try
            {
                ...
                // Get XML Node
                XmlNode xmlNode = doc.SelectSingleNode("Agents/Agent");
                            ...
                            if (notes.StartsWith("AgentID=", StringComparison.OrdinalIgnoreCase) ){
                                ...
                                if (long.TryParse( number.ToString(), out agentID)) {
                                    using (var svc = Brierley.FrameWork.Data.LWDataServiceUtil.CSServiceInstance())
                                    {
                                        ...
                                        {
                                            CSAgent agente = svc.GetCSAgentById(agentID);

                                            if (agente != null)
                                            {
                                                agFirstName = agente.FirstName;
                                                agLastName = agente.LastName;
                ...
                if (null != xmlNode)
                {
                    ...
                    if ( NbrAgentid != this.LastAgentId )
                    {
                        this.printNewAgent = true;
                        this.LastAgentId = NbrAgentid;
                    }
                    else {
                        this.printNewAgent = false;
                    }


                    if ( this.printNewAgent )
                    {
                        lineCount += this.printAgent(agFirstName, agLastName);
                        this.printNewAgent = false;
                    }

                    lineCount +=  this.printRow( awardDate, points, eventName+"/"+pointName, mbrtFirstName +','+mbrLastName);
                }
        ...
        private int printAgent( string firstname, string lastname ) {

            StreamWriter sw = new StreamWriter(this.outputFileName, true);

            sw.WriteLine();
            sw.WriteLine();
            sw.WriteLine("Agent:"+ firstname+","+lastname);
            sw.WriteLine("Date         Points           Point Type/Event                            Member/Reason");
            sw.WriteLine("---------------------------------------------------------------------------------------------------------------------------");
            sw.Flush();
            sw.Close();
        ...
        private int printRow ( DateTime awardDate, double points, string eventName, string mbrName ) {
