...
namespace AmericanEagle.SDK.OutputProviders
{
    ...
    /// </summary>
    public class VendorEmailValidation : IDAPOutputProvider
    {
       
        private string className = MethodBase.GetCurrentMethod().DeclaringType.Name;

        private string outfilename = "AEORW_EM_Welcome_YYYYMMDDHHMMSS.txt";
        ...
        /// </summary>
        private string emailAddr = string.Empty;
        private string eid = string.Empty;

        private string path = string.Empty;
        private DateTime processDate = DateTime.Now;

        private List<string> ValidatedEmailList;  //AEO-418 - EHP
        ...
        public void Initialize(NameValueCollection globals, NameValueCollection args, long jobId, DAPDirectives config, NameValueCollection parameters, DAPPerformanceCounterUtil performUtil)
        {
            ...
            this.ValidatedEmailList = new List<string>(); //AEO-418 - EHP
        ...
        public void ProcessMessageBatch(IList<string> messageBatch)
        {
            ...
            try
            {
                ...
                if (null != xmlNode)
                {
                    ...
                    emailAddr = (xmlNode.Attributes["Email_Address"].Value).Trim().ToLower();
                    eid = xmlNode.Attributes["EID"].Value;

                    if (!string.IsNullOrEmpty(strLoyaltyNumber) )
                    {
                        //AEO-418 - EHP   Starts --->|
                        bool alreadyProcessed = this.ValidatedEmailList.Contains(strLoyaltyNumber);
                        if (alreadyProcessed)
                            throw new Exception( String.Format("LoyaltyNumber:'{0}' is duplicated in file since it has been processed already.", strLoyaltyNumber));
                        else
                            this.ValidatedEmailList.Add(strLoyaltyNumber);
                        //|<--- Ends AEO-418 - EHP
                        ...
                        if (memberDetails != null)
                        {
                            ...
                            if (this.emailAddr.ToUpper() == memberDetails.EmailAddress.Trim().ToUpper())
                            {

                                memberDetails.PendingEmailVerification = 0;
                                memberDetails.NextEmailReminderDate = null;
                                memberDetails.AITUpdate = true;
                                memberDetails.ChangedBy = this.jobname != string.Empty ? this.jobname : "DAP Mail Validation File Load";


                                LWDataServiceUtil.DataServiceInstance(true).SaveMember(member);

                                PointEvent pevent = LWDataServiceUtil.DataServiceInstance(true).GetPointEvent("E-mail Address Bonus");
                                ...
                                if (pevent != null && ptype != null && ptypeNew != null)
                                {
                                    ...
                                    if ((points > 0))
                                    {
                                        ...
                                        aNote.Note = "Email Bonus exceeds more than 1 in 30 days.";
                                    ...
                                    {
                                        Utilities.AddEmailBonus(vc, memberDetails, "EmailBonusPointsPilot");
                                    ...
                                    if (memberDetails.ExtendedPlayCode == 1)
                                    {
                                        ...
                                        string EDIInitial = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("EID_EmailVerification_Initial").Value;
                                        ...
                                        string EIDCredit = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("EID_EmailWelcome_Credit").Value;
                                        string EIDNoCredit = LWDataServiceUtil.DataServiceInstance(true).GetClientConfiguration("EID_EmailWelcome_NoCredit").Value;
                                ...
                                {
                                    throw new Exception("No pointtype and pointevent combination where found for [Bonus Points] and [E-mail Address Bonus]");
                                }

                            }
                            else
                                throw new Exception(String.Format("Email provided in file doesn't match for LoyaltyNumber: '{0}'",  this.strLoyaltyNumber));
