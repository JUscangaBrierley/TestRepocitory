// AEO-5137 [Francisco Sosa Herrera]
...
{
    class ProfileInUpdateInterceptor : IInboundInterceptor
    {
        #region Fields
        ...
        public virtual Member ProcessMemberBeforeSave(LWIntegrationConfig config, Member member, XElement memberNode)
        {
            ...
            string newaddrline1 = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/ADDRESSLINEONE");
            string newaddrline2 = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/ADDRESSLINETWO");
            string city = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/CITY");
            string state = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/STATEORPROVINCE");
            string zipcode = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/ZIPORPOSTALCODE");
            string primaryzip = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/PRIMARYPOSTALCODETMP");
            string country = LWIntegrationUtilities.GetValueByPath(memberNode, "Member/COUNTRY");
            ...
            contVC = member.LoyaltyCards.Count;

            for (int i = 0; i < contVC; i++)
            {
                member.LoyaltyCards[i].LinkKey = long.Parse(LWIntegrationUtilities.GetValueByPath(memberNode, "Member/VirtualCard/LINKKEY"));
            }
            ...
            {                
                ...
                {
                    // detail.SetAttributeValue("AddressLineOne", newaddrline1);
                    if (!string.IsNullOrEmpty(newaddrline1))
                        member.GetChildAttributeSets("MemberDetails")[x].SetAttributeValue("AddressLineOne", newaddrline1);
                    if (!string.IsNullOrEmpty(newaddrline2))
                        member.GetChildAttributeSets("MemberDetails")[x].SetAttributeValue("AddressLineTwo", newaddrline2);
                    if (!string.IsNullOrEmpty(city))
                        member.GetChildAttributeSets("MemberDetails")[x].SetAttributeValue("City", city);
                    if (!string.IsNullOrEmpty(state))
                        member.GetChildAttributeSets("MemberDetails")[x].SetAttributeValue("StateOrProvince", state);
                    if (!string.IsNullOrEmpty(zipcode))
                        member.GetChildAttributeSets("MemberDetails")[x].SetAttributeValue("ZipOrPostalCode", zipcode);
                    if (!string.IsNullOrEmpty(primaryzip))                        
                        member.PrimaryPostalCode = primaryzip;
                    if (!string.IsNullOrEmpty(country))
                        member.GetChildAttributeSets("MemberDetails")[x].SetAttributeValue("Country", country);
