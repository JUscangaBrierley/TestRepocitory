...
public partial class SocialShare : System.Web.UI.Page
{
	...
	private static void SendRewardMarkup(NameValueCollection args, HttpRequest Request, HttpResponse Response) 
	{
		...
		ILWDataService dataService = PortalState.DataManager;
		MemberReward memberReward = dataService.GetMemberReward(rewardID);
		string markup = GetMarkup(Request, memberReward.RewardDef.Name, memberReward.RewardDef.ShortDescription, memberReward.RewardDef.LogoFileName);
	...
	private static void SendCouponMarkup(NameValueCollection args, HttpRequest Request, HttpResponse Response)
	{
		...
		ILWDataService dataService = PortalState.DataManager;
		MemberCoupon memberCoupon = dataService.GetMemberCoupon(couponID);
		CouponDef couponDef = dataService.GetCouponDef(memberCoupon.CouponDefId);
