...
namespace Brierley.LoyaltyWare.LWMobileGateway.OperationProviders.Messages
{
    ...
    {
        private const string _className = "GetMessageDefinitions";
        ...
        public override object Invoke(string source, WcfAuthenticationToken token, object[] parms)
        {
            ...
            if (messages.Count == 0)
            {
                throw new LWOperationInvocationException("No message definitions found.") { ErrorCode = 3362 };
            }

            List<MGMessageDef> msgList = null;

            if (messages.Count > 0)
            {
                msgList = new List<MGMessageDef>();
                foreach (MessageDef message in messages)
                {
                    msgList.Add(MGMessageDef.Hydrate(message, language, channel, returnAttributes));
                }
            }
