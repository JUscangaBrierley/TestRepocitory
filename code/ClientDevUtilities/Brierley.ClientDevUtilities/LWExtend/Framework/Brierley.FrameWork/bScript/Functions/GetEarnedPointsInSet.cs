...
namespace Brierley.FrameWork.bScript.Functions
{
	...
	/// </remarks>
	[Serializable]
	[ExpressionContext(Description = "Returns the member's earned points value for the given date range and the set of point types",
	DisplayName = "GetEarnedPointsInSet",
	...
	WizardDescription = "Earned points by types and events",
	...
	[ExpressionParameter(Order = 1, Name = "Point Events", Type = ExpressionApplications.Strings, Optional = false, WizardDescription = "Which point events?", Helpers = ParameterHelpers.PointEvent, AllowMultiple = true)]
	[ExpressionParameter(Order = 2, Name = "Start Date", Type = ExpressionApplications.Dates, Optional = false, WizardDescription = "From which date?")]
	[ExpressionParameter(Order = 3, Name = "End Date", Type = ExpressionApplications.Dates, Optional = false, WizardDescription = "To which date?")]
	...
	public class GetEarnedPointsInSet : UnaryOperation
	{
		...
		internal GetEarnedPointsInSet(Expression rhs)
			: base("GetEarnedPointsInSet", rhs)
		{
			...
			if (plist != null)
			{
				...
				//else
				//{
				//    throw new LWBScriptException("Invalid Function Call: Wrong number of arguments passed to GetEarnedPoints.");
				//}
			}
			else
			{
		...
		public new string Syntax
		{
			get
			{
				return "GetPointsInSet('PointType1;PointType2;PointType3','PointEvent1;PointEvent2;PointEvent3', Start Date, End Date,[Include Expired Points])";
			}
		}
		...
		public override object evaluate(ContextObject contextObject)
		{
			...
			if (member != null)
			{
				using (var service = LWDataServiceUtil.LoyaltyDataServiceInstance())
				{
					...
					{
						throw new CRMException("GetEarnedPointsInSet: end date is less than the start date.");
					}
					...
					return service.GetEarnedPointBalance(member.LoyaltyCards, typeIds, eventIds, startDate, endDate, includeExpiredPoints);
				}
			}
			else
			{
				throw new CRMException("GetEarnedPointsInSet must be evaluated in the context of a loyalty member.");
			}
		}
	}
}
