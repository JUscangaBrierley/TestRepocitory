...
namespace Brierley.FrameWork.WalletPay
{
    public class WobUtils
    {
        ...
        {
            issuer = iss;
            var logger = Brierley.FrameWork.Common.Logging.LWLoggerManager.GetLogger(Brierley.FrameWork.Common.LWConstants.LW_FRAMEWORK);
            if (HttpContext.Current != null)
            {
                System.Uri currentUri = HttpContext.Current.Request.Url;
                origins = new string[] { string.Format("{0}://{1}", currentUri.Scheme, currentUri.Authority) };
            }
            else // temporary code for .Net Core
            {
                origins = new string[] { "http://www.google.com" };
            }
