...
{
    internal class ArchiveObjectMapper : IMapper
    {
        private PetaPoco.StandardMapper standardMapper = new PetaPoco.StandardMapper();

        public Func<object, object> GetFromDbConverter(PropertyInfo pi, Type sourceType)
        {
            if (pi.PropertyType == typeof(SupportedObjectType))
            {
                return src =>
                {
                    return (SupportedObjectType)Enum.Parse(typeof(SupportedObjectType), src.ToString());
                };
            }
            return null;
        }
