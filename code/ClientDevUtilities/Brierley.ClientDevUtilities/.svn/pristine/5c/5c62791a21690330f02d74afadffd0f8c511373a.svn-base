...
namespace PetaPoco
{
	...
	*/
	...
	public class ColumnAttribute : Attribute
	{
        ...
        // Additional Properties added for proper schema generation
	...
	namespace Internal
	{
		...
		abstract class DatabaseType
		{
			...
			public static DatabaseType Resolve(string TypeName, string ProviderName)
			{
                ...
                if (TypeName.StartsWith("System.Data.SqlClient.") || TypeName.StartsWith("SqlClient")) //Added the "SqlClient" to pick up the SQLClient factory
		...
		class PocoData
		{
			...
			private static Func<object, object> GetConverter(IMapper mapper, PocoColumn pc, Type srcType, Type dstType)
			{
				...
				else if (!dstType.IsAssignableFrom(srcType))
				{
                    ...
                    // Handle integral -> nullable enum
                    else if (Nullable.GetUnderlyingType(dstType) != null && Nullable.GetUnderlyingType(dstType).IsEnum && IsIntegralType(srcType))
                    {
                        // We have to create an actual enumeration object because MSIL won't convert anything but the base type to its nullable type
                        return delegate(object src) { return src == null ? null : Enum.ToObject(Nullable.GetUnderlyingType(dstType), src); };
                    }
                    // Just try to convert it - making sure we get the non-nullable type
                    else
                    {
                        return delegate(object src)
                        {
                            Type t = Nullable.GetUnderlyingType(dstType) ?? dstType;
		...
		class OracleDatabaseType : DatabaseType
		{
			...
			public override string BuildPageQuery(long skip, long take, PagingHelper.SQLParts parts, ref object[] args)
			{
                ...
                var sqlPage = string.Format("SELECT * FROM (SELECT ROW_NUMBER() OVER ({0}) peta_rn, {1}) peta_paged WHERE peta_rn>@{2} AND peta_rn<=@{3}",
                                        parts.sqlOrderBy == null ? "ORDER BY (SELECT NULL FROM dual)" : parts.sqlOrderBy, parts.sqlSelectRemoved, args.Length, args.Length + 1);
