...
{
	public class DmcTriggeredEmailRetry : DmcTriggeredEmail
	{
		private const string _className = "TriggeredEmailRetry";
		private static LWLogger _logger = LWLoggerManager.GetLogger(LWConstants.LW_FRAMEWORK);

		public DmcTriggeredEmailRetry()
			: base(new NullCommunicationLogger())
		{
			Config = LWConfigurationUtil.GetCurrentConfiguration();
		}

		public DmcTriggeredEmailRetry(long emailId)
			: base(new NullCommunicationLogger())
		{
			MailingId = emailId;
		}
		...
		public bool Resend(long emailQueueID)
		{
			...
			using (var svc = LWDataServiceUtil.EmailServiceInstance(Config.Organization, Config.Environment))
			{
				try
				{
					...
					//serialNumber = Send(emailQueue.Records, emailQueue, out resendEmailQueueID);
					...
					SendDmcAsync(data.User.Email, data.Personalizations).Wait();
