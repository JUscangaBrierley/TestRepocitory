...
namespace Brierley.FrameWork.bScript.Functions
{
	...
	/// </remarks>
	[Serializable]
	[ExpressionContext(Description = "Returns the member's points value for the given date range and excluding the point types in the provided set.",
	DisplayName = "GetPointsExcludingSet",
	...
	public class GetPointsExcludingSet : UnaryOperation
	{
		...
		internal GetPointsExcludingSet(Expression rhs)
			: base("GetPointsExcludingSet", rhs)
		{
		...
		public new string Syntax
		{
			get
			{
				return "GetPointsExcludingSet('PointType1;PointType2;PointType3',Start Date, End Date)";
			}
		}
		...
		public override object evaluate(ContextObject contextObject)
		{
			...
			if (member != null)
			{
				using (var service = LWDataServiceUtil.LoyaltyDataServiceInstance())
				{
					...
					if (typeIds != null)
					{
						return member.GetPointsExcludingPointTypes(startDate, endDate, typeIds);
					}
					else
					{
						throw new CRMException("GetPointsExcludingSet must have type ids to evaluate.");
					}
				}
			}
			else
			{
				throw new CRMException("GetPointsExcludingSet must be evaluated in the context of a loyalty member.");
			}
		}
	}
}
