...
{
    public class GetMemberRewardByCert : OperationProviderBase
    {
        #region Fields
        private const string _className = "GetMemberRewardByCert";
        private LWLogger _logger = LWLoggerManager.GetLogger(LWConstants.LW_MOBILEGATEWAY_SERVICE);
        #endregion

        public GetMemberRewardByCert() : base("GetMemberRewardByCert") { }
        ...
        public override object Invoke(string source, WcfAuthenticationToken token, object[] parms)
        {
            ...
            string certNumber = (string)parms[0];
            if (!string.IsNullOrEmpty(certNumber))
            {
                string errMsg = "No reward id provided for GetMemberRewardByCert.";
                _logger.Error(_className, methodName, errMsg);
                throw new LWOperationInvocationException(errMsg) { ErrorCode = 1 };
            }

            MemberReward reward = LoyaltyService.GetMemberRewardByCert(certNumber);
            if (reward == null)
            {
                throw new LWOperationInvocationException("No reward found with cert " + certNumber + ".") { ErrorCode = 3347 };
            }
