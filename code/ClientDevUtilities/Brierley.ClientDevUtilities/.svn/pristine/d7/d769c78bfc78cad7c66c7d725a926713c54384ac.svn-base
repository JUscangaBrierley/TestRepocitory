...
{
    public class GetGeneralSurveys : OperationProviderBase
    {
        #region Fields
        private const string _className = "GetGeneralSurveys";
        private LWLogger _logger = LWLoggerManager.GetLogger(LWConstants.LW_MOBILEGATEWAY_SERVICE);
        #endregion

        public GetGeneralSurveys() : base("GetGeneralSurveys") { }
        ...
        public override object Invoke(string source, WcfAuthenticationToken token, object[] parms)
        {
            ...
            {
                string errMsg = "Invalid parameters provided for GetGeneralSurveys.";
            ...
            List<MGSurvey>  results = MGSurveyManager.GetSurveys(language, SurveyType.General, member);
            if (results == null || results.Count == 0)
            {
                throw new LWOperationInvocationException("No general surveys found.") { ErrorCode = 3362 };
            }
