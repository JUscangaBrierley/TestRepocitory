CREATE OR REPLACE PACKAGE AEPointBalance2 IS
  ...
  PROCEDURE CalculatePointBalances_2(p_Jobname     VARCHAR2,
                                   p_ProcessDate Date,
                                   Retval        IN OUT Rcursor);
 PROCEDURE CalculatePointBalances_3(p_Jobname     VARCHAR2,
                                   p_ProcessDate Date,
                                   Retval        IN OUT Rcursor);
...
CREATE OR REPLACE PACKAGE BODY AEPointBalance2 IS
...
PROCEDURE CalculatePointBalances_1(p_Jobname     VARCHAR2,
  ...
  BEGIN
    ...
    v_sql := 'Create Table AE_CurrentPointTransaction2 As Select * from lw_pointtransaction pt Where pt.transactiondate > Add_Months(trunc(sysdate, ''Q''), -3)';
      ...
      SELECT /*+ append full (vc) full (md) full (Pts) NO_PUSH_PRED(Lw_Clientconfiguration) NO_PUSH_PRED(Ats_Memberbrapromocertsummary) NO_PUSH_PRED(Ats_Memberbrapromocerthistory) NO_PUSH_PRED(Ats_Memberbrapromosummary) NO_PUSH_PRED(Ats_Memberbrand)*/
       ...
       SUM(CASE
             WHEN Ty.Name IN ('Basic Points', 'Adjustment Points') AND
                 ...
                 Trunc(Pts.Transactiondate) BETWEEN Trunc(p_ProcessDate, 'Q') AND Trunc(p_ProcessDate ) THEN
              Pts.Points
             ELSE
              0
           END) AS Basic_Points,
...
PROCEDURE CalculatePointBalances_2(p_Jobname     VARCHAR2,
  ...
  END CalculatePointBalances_2;
   ...
   /********************************************************************
  **This is the parallel process another point transaction table for 03/31/14 issue of bad memberpoints file being sent
  ********************************************************************/
PROCEDURE CalculatePointBalances_3(p_Jobname     VARCHAR2,
                                   p_ProcessDate Date,
                                   Retval        IN OUT Rcursor)
   ...
   AS
    ...
    v_Messagesfailed   NUMBER := 0;
    v_Jobstatus        NUMBER := 0;
    v_Jobname          VARCHAR2(256) := 'CalculatePointBalances_3';
    v_RewardLevel_15   NUMBER;
    v_RewardLevel_20   NUMBER;
    v_RewardLevel_30   NUMBER;
    v_RewardLevel_40   NUMBER;
  ...
  BEGIN
    ...
    EXECUTE IMMEDIATE 'Drop Table AE_CurrentPointTransaction';

    v_sql := 'Create Table AE_CurrentPointTransaction As Select * from lw_pointtransaction pt Where pt.transactiondate > Add_Months(trunc(sysdate, ''Q''), -3)';
    ...
    EXECUTE IMMEDIATE 'Truncate Table ae_memberpointbalances_010114';

    INSERT INTO ae_memberpointbalances_010114 pb
      (a_Rowkey,
       a_Ipcode,
       a_Parentrowkey,
       a_Previoustotalpoints,
