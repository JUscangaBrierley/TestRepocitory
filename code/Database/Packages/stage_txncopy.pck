CREATE OR REPLACE PACKAGE STAGE_TXNCOPY IS
type rcursor IS REF CURSOR;
  /*
PI29741 tlog optimization changes begin here                 -----scj
*/
      PROCEDURE StageToTxnHeadWrk(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailWrk(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnHeader(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetail(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnRedeem(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailDiscount(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTender(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTables(p_Processdate VARCHAR2,p_email_list VARCHAR2);
      PROCEDURE TlogInterceptorProcessing(p_Dummy VARCHAR2,retval IN OUT rcursor);
      PROCEDURE UpdateMemberAttributeFromTlog(p_Processdate VARCHAR2,retval IN OUT rcursor);
      ...
      PROCEDURE TxnHeaderWrkBkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE TxnDetailWrkBkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
...
END STAGE_TXNCOPY;
/
CREATE OR REPLACE PACKAGE BODY STAGE_TXNCOPY   IS

PROCEDURE StageToTxnHeadWrk(p_Dummy      VARCHAR2,
...
BEGIN
  ...
  v_jobname   := 'TLog1-TxnHeadWrk_Staging';
  ...
  BEGIN
    ...
    If v_headerwkcount > 0 then
      Delete from LW_txnheader_Wrk; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
        ...
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeadWrk: ';
          ...
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeadWrk: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeadWrk</proc>' ||
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk1 txncopy failures DML ERROR ' ||
                                             sysdate,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeadWrk</proc>' || Chr(10) ||
      ...
      cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                    ...
                    p_bcc_list      => NULL,
                    p_subject       => v_envkey ||
                                       ' American Eagle Tlog Chunk1 txncopy failure on ' ||
                                       sysdate,
                    p_text_message  => v_message,
                    p_content_type  => 'text/html;charset=UTF8',
                    p_attachments   => v_attachments, --v_attachments,
                    ...
                    p_auth_password => NULL,
                    p_mail_server   => 'cypwebmail.brierleyweb.com');
      Raise_Application_Error(-20002, 'STAGE_TXNCOPY job is not finished');
  END;
END StageToTxnHeadWrk;

PROCEDURE StageToTxnDetailWrk(p_Dummy      VARCHAR2,
                              p_email_list VARCHAR2,
                              retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TxnDetailWrk_Staging';
  v_logsource := v_jobname;
  ...
  BEGIN
    ...
    If v_detailitemwkcount > 0 then
      Delete from LW_txndetailitem_Wrk; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
    OPEN get_data;
    LOOP
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailWrk: ';
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailWrk: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailWrk</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk1 txncopy failure DML ERROR ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailWrk</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
      ...
      Raise_Application_Error(-20002, 'STAGE_TXNCOPY job is not finished');
  END;
END StageToTxnDetailWrk;

PROCEDURE StageToTxnHeader(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_Header_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk hw
         --AEO-858 BEGIN
         where 1=1
         and not exists
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeader: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeader: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeader</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeader</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
END StageToTxnHeader;

PROCEDURE StageToTxnDetail(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_Detailitem_Staging';
  v_logsource := v_jobname;
    ...
    LOOP
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetail: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetail</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetail</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetail;

PROCEDURE StageToTxnRedeem(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_Redeem_Staging';
  v_logsource := v_jobname;
    ...
    CURSOR get_data IS
      WITH Mem AS
       (select rr.*, hw.header_rowkey
          from LW_TXNREWARDREDEEM_STAGE rr
         inner join LW_txnheader_Wrk hw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnRedeem: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnRedeem: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnRedeem</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnRedeem</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnRedeem;

PROCEDURE StageToTxnDetailDiscount(p_Dummy      VARCHAR2,
                                   p_email_list VARCHAR2,
                                   retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_DetailDiscount_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       from LW_TXNDETAILDISCOUNT_STAGE dds
       inner join LW_txndetailitem_Wrk dw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailDiscount: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailDiscount: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailDiscount</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailDiscount</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailDiscount;

PROCEDURE StageToTxnTender(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_Tender_Staging';
  v_logsource := v_jobname;
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnTender: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnTender: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnTender</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk1 txncopy failure DML ERRORS ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnTender</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnTender;
PROCEDURE StageToTxnTables(p_Processdate VARCHAR2,p_email_list VARCHAR2) AS
   p_Dummy2 VARCHAR2(1);
  crsr STAGE_TXNCOPY.rcursor;
BEGIN

  DECLARE
  BEGIN
   StageToTxnHeadWrk(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailWrk(p_Dummy2,p_email_list,crsr);
   Giftcard24HrPurchase(p_Dummy2,crsr);
   BasicReturns24HrAdjust(p_Dummy2,crsr);
   TxnHeaderWrkBkp(p_Dummy2,p_email_list,crsr);
   TxnDetailWrkBkp(p_Dummy2,p_email_list,crsr);
   StageToTxnHeader(p_Dummy2,p_email_list,crsr);
   StageToTxnDetail(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailDiscount(p_Dummy2,p_email_list,crsr);
   StageToTxnRedeem(p_Dummy2,p_email_list,crsr);
   StageToTxnTender(p_Dummy2,p_email_list,crsr);
   --ReturnRedeemTxnPoints(p_Processdate ,p_email_list,crsr);
   END;
END StageToTxnTables;

PROCEDURE TlogInterceptorProcessing(p_Dummy VARCHAR2, Retval  IN OUT Rcursor) AS
  ...
  BEGIN
     v_jobname := 'TLog1-TlogInterceptorProcessing';
     v_logsource := v_jobname;
...
--   v_dap_log_id := utility_pkg.get_LIBJobID();
  ...
  utility_pkg.Log_job(P_JOB                => v_my_log_id
         ...
         ,p_jobstatus          => v_jobstatus
         ,p_jobname            => v_jobname);
 utility_pkg.Log_Process_Start(v_jobname, 'TlogInterceptorProcessing', v_processId);
 /*Logging*/
 DECLARE
   CURSOR get_data IS
      WITH Mem AS
      ...
      from lw_txndetailitem_wrk tw
      inner join lw_txnheader_wrk hw on hw.txnheaderid = tw.txnheaderid
         ...
         -- For those records whose brands are not set, check each product at detail item
          IF  (bcheckitembrand = 1) then
                   ...
                   If (v_DtlMemberHasBrand = 0) Then
                            ...
                            v_Message        := 'TlogInterceptorProcessing is not finished';
...
--  END IF;
   v_endtime := SYSDATE;
    v_Messagesfailed := v_Messagesfailed + 1;
    v_Error          := SQLERRM;
    v_Reason         := 'Failed Procedure TlogInterceptorProcessing: ';
    v_Message        := '<failed>' || Chr(10) ||
                        '  <details>' || Chr(10) ||
                        '    <pkg>STAGE_TXNCOPY</pkg>' ||
                        Chr(10) ||
                        '    <proc>TlogInterceptorProcessing</proc>' ||
                        Chr(10) || '  </details>' ||
                        Chr(10) || '</failed>';
...
END TlogInterceptorProcessing;

    /********************************************************************
  ********************************************************************/
  PROCEDURE UpdateMemberAttributeFromTlog(p_Processdate VARCHAR2, Retval  IN OUT Rcursor) AS
    v_Logsource        VARCHAR2(256) := 'STAGE_TXNCOPY.UpdateMemberAttributesFromTlog';
    v_My_Log_Id        NUMBER;
    v_Jobdirection     NUMBER := 0;
    v_Filename         VARCHAR2(512) := 'STAGE_TXNCOPY.UpdateMemberAttributesFromTlog';
    v_Starttime        DATE := SYSDATE;
    v_Endtime          DATE;
    v_Messagesreceived NUMBER := 0;
    v_Messagesfailed   NUMBER := 0;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                          ...
                          p_Msgtime   => SYSDATE);
   Raise;
  END UpdateMemberAttributeFromTlog;
...
BEGIN
 ...
 --  v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog1-Giftcard24HrPurchase';
     v_logsource := v_jobname;
  ...
  BEGIN
    ...
    LOOP
      FETCH get_data BULK COLLECT
        INTO v_tbl LIMIT 10000;
    FORALL j IN 1 .. v_tbl.COUNT -- SAVE EXCEPTIONS
        UPDATE  LW_txndetailitem_Wrk DW
...
-- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog1-BasicReturns24HrAdjust';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'BasicReturns24HrAdjust', v_processId);
  DECLARE
      ...
      WITH Mem AS
       (
       select hw.txnheaderid from lw_txnheader_wrk hw
...
PROCEDURE TxnHeaderWrkBkp(p_Dummy      VARCHAR2,
                          p_email_list VARCHAR2,
                          retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_Header_Bkp';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk wk
       --AEO-858 BEGIN
       where 1=1
       and not exists
         (select 1 from LW_txnheader_WrkBkp wb where wb.txnheaderid = wk.txnheaderid) -- Checking for duplicate txn header records
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txnheader_WrkBkp
            (rowkey,
             header_rowkey,
             vckey,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnHeaderWrkBkp: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnHeaderWrkBkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) || '    <proc>TxnHeaderWrkBkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>TxnHeaderWrkBkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
PROCEDURE TxnDetailWrkBkp(p_Dummy      VARCHAR2,
                          p_email_list VARCHAR2,
                          retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog1-TXN_Detailitem_WrkBkp';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       --AEO-858 BEGIN
       and not exists
       (select 1 from LW_txndetailitem_WrkBkp db where db.txndetailid = dw.txndetailid) -- Checking for duplicate txn detailitem
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_WrkBkp
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnDetailWrkBkp: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnDetailWrkBkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY</pkg>' ||
                              Chr(10) || '    <proc>TxnDetailWrkBkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY</pkg>' || Chr(10) ||
                   '    <proc>TxnDetailWrkBkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog-ReturnRedeemTxnPoints';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'ReturnRedeemTxnPoints wk', v_processId);
  DECLARE
...
WHEN OTHERS THEN
           ...
           cio_mail.send(
                ...
                p_cc_list       => NULL,
                p_bcc_list      => NULL,
                p_subject       =>  v_envkey ||' American Eagle Tlog Chunk1 txncopy failure on ' || sysdate  ,
                p_text_message  => v_message,
                p_content_type  => 'text/html;charset=UTF8',
                p_attachments   => v_attachments, --v_attachments,
                p_priority      => '3',
