CREATE OR REPLACE PACKAGE Stage_Tlog IS
  ...
  */
  ...
  PROCEDURE Stage_Tlog01(p_Filename   VARCHAR2 DEFAULT NULL,
  ...
  PROCEDURE Hist_Tlog01_ByJobNumber(p_Dummy VARCHAR2, p_JobNumber NUMBER,
                        Retval  IN OUT Rcursor);

  PROCEDURE Clear_Stage(p_Dummy VARCHAR2,
                        Retval  IN OUT Rcursor);

  PROCEDURE Stage_unprocessed_Tlog01 (p_Dummy VARCHAR2,
                                      Retval       IN OUT Rcursor);
  PROCEDURE Re_ProcessStagedTlog(p_Dummy VARCHAR2,
                                 Retval  IN OUT Rcursor);
  PROCEDURE log_unprocessed_Tlog01 (p_JobNumber NUMBER DEFAULT -1); /* this called by hist_tlog, so dont run manually*/
  ...
  CURSOR Get_Discounts(p_Header_Id  VARCHAR2,
    ...
    SELECT Rowkey,
           ...
           Lastdmlid,
           Fileid
      FROM Lw_Txndetaildiscount_Stage
...
END Stage_Tlog;
/
CREATE OR REPLACE PACKAGE BODY Stage_Tlog IS
   gv_Detail_Tbl     Type_Tlog01_Stg_Detail_Tbl := Type_Tlog01_Stg_Detail_Tbl();
   gv_Tender_Tbl     Type_Tlog01_Stg_Tender_Tbl := Type_Tlog01_Stg_Tender_Tbl();
   gv_Discount_Tbl   Type_Tlog01_Stg_Discount_Tbl := Type_Tlog01_Stg_Discount_Tbl();
   gv_Reward_Tbl     Type_Tlog01_Stg_Reward_Tbl  := Type_Tlog01_Stg_Reward_Tbl();
...
FUNCTION is_duplicated(p_txnheaderid  number) RETURN number IS
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Get_Tlog01_00header(p_Rec IN VARCHAR2) RETURN Type_Tlog01_00header IS
    v_Return_Rec Type_Tlog01_00header;
    /*
  ...
  BEGIN
    IF p_Rec LIKE '00%'
    THEN
      v_Return_Rec := Type_Tlog01_00header(Substr(p_Rec, 001, 2),
                                           Substr(p_Rec, 003, 5),
                                           Substr(p_Rec, 008, 3),
                                           Substr(p_Rec, 011, 9),
                                           Substr(p_Rec, 020, 6),
  ...
  ********************************************************************/
    ...
    FUNCTION Get_Tlog01_01item(p_Rec IN VARCHAR2) RETURN Type_Tlog01_01item IS
    v_Return_Rec    Type_Tlog01_01item;
...
-- PI 31647 - 77kids class codes reassigned to AE - points not added - Start
    ...
    IF p_Rec LIKE '01%'
    THEN
      v_Return_Rec := Type_Tlog01_01item(Substr(p_Rec, 001, 2),
                                         Substr(p_Rec, 003, 1),
                                         Substr(p_Rec, 004, 9),
  ...
  ********************************************************************/
    ...
    FUNCTION Get_Tlog01_99item(p_Rec IN VARCHAR2) RETURN Type_Tlog01_99item IS
    v_Return_Rec    Type_Tlog01_99item;
    v_Actual_Amount NUMBER;
    v_Retail_Amount NUMBER;
    v_Quantity      NUMBER;
    v_77Kids_ClassCode NUMBER;
  ...
  BEGIN
    ...
    IF p_Rec LIKE '99%'
    THEN
      v_Return_Rec := Type_Tlog01_99item(Substr(p_Rec, 001, 2),
                                         Substr(p_Rec, 003, 1),
                                         Substr(p_Rec, 004, 9),
                                         Ltrim(TRIM(Substr(p_Rec, 013, 12)),
                                               '0') /* sku */,
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_02subtotal(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_02subtotal IS
    v_Return_Rec Type_Tlog01_02subtotal;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog01_02subtotal(Substr(p_Rec, 001, 2),
                                           Substr(p_Rec, 003, 1),
                                           v_Amount);
    /*  tlog record spec
    (STRING_TYPE               varchar2(2)   \*001 - 002*\
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_03salestax(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_03salestax IS
    v_Return_Rec Type_Tlog01_03salestax;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog01_03salestax(Substr(p_Rec, 001, 1),
                                           Substr(p_Rec, 003, 1),
                                           Substr(p_Rec, 004, 1),
                                           v_Amount);
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_04tender(p_Rec IN VARCHAR2) RETURN Type_Tlog01_04tender IS
    v_Return_Rec Type_Tlog01_04tender;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog01_04tender(Substr(p_Rec, 001, 2),
                                         Substr(p_Rec, 003, 2),
                                         Substr(p_Rec, 005, 2),
                                         v_Amount);
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_05giftcert(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_05giftcert IS
    v_Return_Rec Type_Tlog01_05giftcert;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog01_05giftcert(Substr(p_Rec, 001, 2),
                                           Substr(p_Rec, 003, 1),
                                           Substr(p_Rec, 014, 9),
                                           Substr(p_Rec, 013, 5),
                                           v_Amount);
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_14foundorder(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_14foundorder IS
    v_Return_Rec Type_Tlog01_14foundorder;
    v_Amount     NUMBER;
    v_Taxrate    NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
  ...
  --  v_Taxrate    := To_Number(Substr(p_Rec, 012, 5)) * .001; /*(Commented to avoid 10/26 tlog issue ,if tax is double digit,then exceeds column length - char2(5)) */
    v_Return_Rec := Type_Tlog01_14foundorder(Substr(p_Rec, 001, 2),
                                             Substr(p_Rec, 003, 1),
                                             Substr(p_Rec, 004, 1),
                                             Substr(p_Rec, 005, 1),
                                             Substr(p_Rec, 006, 1),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_15ceditcrdcheck(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_15ceditcrdcheck IS
    v_Return_Rec Type_Tlog01_15ceditcrdcheck;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog01_15ceditcrdcheck(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 2),
                                                Substr(p_Rec, 005, 27),
                                                Substr(p_Rec, 032, 4),
                                                Substr(p_Rec, 036, 4),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_18txndiscount(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_18txndiscount IS
    v_Return_Rec Type_Tlog01_18txndiscount;
    /*
      Parses tlog rec and loads into formatted array.
    */
    v_Amount1 NUMBER;
  ...
  BEGIN
    ...
    v_Return_Rec := Type_Tlog01_18txndiscount(Substr(p_Rec, 001, 2),
                                              Substr(p_Rec, 003, 1),
                                              Substr(p_Rec, 004, 1),
                                              v_Amount1,
                                              v_Amount2);
    ...
    */
    ...
    RETURN v_Return_Rec;
  END Get_Tlog01_18txndiscount;
  /********************************************************************
  ********************************************************************
  ********************************************************************
  ********************************************************************/

  FUNCTION Get_Tlog01_2733origtxn(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2733origtxn IS
    v_Return_Rec Type_Tlog01_2733origtxn;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog01_2733origtxn(Substr(p_Rec, 001, 2),
                                            Substr(p_Rec, 003, 3),
                                            Substr(p_Rec, 006, 1),
                                            Substr(p_Rec, 007, 2),
                                            Substr(p_Rec, 009, 6),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_2751discountrsn(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2751discountrsn IS
    v_Return_Rec Type_Tlog01_2751discountrsn;
    /*
      Parses tlog rec and loads into formatted array.
    */
  ...
  BEGIN
   v_DiscountAmount    := To_Number(NVL(trim(Substr(p_Rec, 076, 10)),0)) * .01;
    /*
    v_Return_Rec := Type_Tlog01_2751discountrsn(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 3),
                                                Substr(p_Rec, 006, 1),
                                                Substr(p_Rec, 007, 2),
                                                Substr(p_Rec, 009, 1),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_2762valuecard(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2762valuecard IS
    v_Return_Rec Type_Tlog01_2762valuecard;
    /*
      Parses tlog rec and loads into formatted array.
    */
  ...
  BEGIN
    ...
    v_Return_Rec  := Type_Tlog01_2762valuecard(Substr(p_Rec, 001, 2),
                                               Substr(p_Rec, 003, 3),
                                               Substr(p_Rec, 006, 1),
                                               Substr(p_Rec, 007, 2),
                                               Substr(p_Rec, 009, 2),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_2777multidivmsttxn(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2777multidivmsttxn IS
    v_Return_Rec Type_Tlog01_2777multidivmsttxn;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog01_2777multidivmsttxn(Substr(p_Rec, 001, 2),
                                                   Substr(p_Rec, 003, 3),
                                                   Substr(p_Rec, 006, 1),
                                                   Substr(p_Rec, 007, 2),
                                                   Substr(p_Rec, 009, 5),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_2792pomogiftsku(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2792pomogiftsku IS
    v_Return_Rec Type_Tlog01_2792pomogiftsku;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog01_2792pomogiftsku(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 3),
                                                Substr(p_Rec, 006, 1),
                                                Substr(p_Rec, 007, 2),
                                                Substr(p_Rec, 009, 12));
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_2793custloyalyt(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2793custloyalyt IS
    v_Return_Rec Type_Tlog01_2793custloyalyt;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog01_2793custloyalyt(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 3),
                                                Substr(p_Rec, 006, 1),
                                                Substr(p_Rec, 007, 2),
                                                Substr(p_Rec, 009, 14));
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog01_2799directordernbr(p_Rec IN VARCHAR2)
    RETURN Type_Tlog01_2799directordernbr IS
    v_Return_Rec Type_Tlog01_2799directordernbr;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog01_2799directordernbr(Substr(p_Rec, 001, 2),
                                                   Substr(p_Rec, 003, 3),
                                                   Substr(p_Rec, 006, 1),
                                                   Substr(p_Rec, 007, 2),
 ...
 /********************************************************************
  ...
  Stage_Tlog01 procedure that will go back through and zero out any
  ...
  ********************************************************************/
  ...
  BEGIN
    ...
    DECLARE
      ...
      and    not exists (select 1 from lw_txndetail_stage stg2 where stg2.txnstoreid = stg.txnoriginalstoreid and trunc(stg2.txndate) = stg.txnoriginaltxndate and stg2.txnnumber = stg.txnoriginaltxnnumber and stg2.ipcode > 0)
      union
            select distinct stg.txnheaderid
      from lw_txndetail_stage stg
      Where  1=1
      ...
      and    not exists (select 1 from lw_txndetail_stage stg2 where stg2.txnstoreid = stg.txnoriginalstoreid and stg2.txnnumber = stg.txnoriginaltxnnumber and stg2.ipcode > 0)
      ;
...
*/
  BEGIN
    ...
    DECLARE
      CURSOR get_data2 IS
        SELECT certificatecode, txndate from lw_txnrewardredeem_stage;
...
/*Redesign Changes begin Here ---------------------------------------------SCJ*/
  ...
  ********************************************************************/
  PROCEDURE UpdateMemberAttributesFromTlog AS
    v_Logsource        VARCHAR2(256) := 'Stage_Tlog.UpdateMemberAttributesFromTlog';
    v_My_Log_Id        NUMBER;
    v_Jobdirection     NUMBER := 0;
    v_Filename         VARCHAR2(512) := 'Stage_Tlog.UpdateMemberAttributesFromTlog';
  ...
  ********************************************************************/
  ...
  BEGIN
    ...
    */
    v_Sql := 'ALTER TABLE ext_tlog01' || Chr(10) || 'LOCATION (AE_IN' ||
             Chr(58) || '''' || p_Filename || ''')';
    EXECUTE IMMEDIATE v_Sql;
    Clear_Infile('ext_tlog01_log');
  END Initialize_Tbl;
  /********************************************************************
  ********************************************************************
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Head_Rec RETURN Type_Tlog01_Stg_Header IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog01_Stg_Header(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Detail_Rec RETURN Type_Tlog01_Stg_Detail IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog01_Stg_Detail(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Tender_Rec RETURN Type_Tlog01_Stg_Tender IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog01_Stg_Tender(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Discount_Rec RETURN Type_Tlog01_Stg_Discount IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog01_Stg_Discount(NULL,
                                    NULL,
                                    NULL,
                                    NULL,
                                    NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Reward_Rec RETURN Type_Tlog01_Stg_Reward IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog01_Stg_Reward(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************/
  FUNCTION Get_24hr_Txn_Cnt(/*p_Vckey IN NUMBER,*/
                            p_ipcode IN NUMBER,
                             ...
                             p_qualamount OUT FLOAT, -- AEO-1748                                                           --- to be changed to a procedure, when need for SQL to call this arises
  ...
  ********************************************************************/
  ...
  PROCEDURE Stage_unprocessed_Tlog01 (p_Dummy VARCHAR2,
    ...
    */
    ...
    v_Jobname          VARCHAR2(256) := 'UnProcessedTLog01';
  BEGIN
    /* data need to be written to stage tables */
    Log_Process_State('Lw_Txndetail_Stage');
...
/* get job log id for this process and for the dap process */
    ...
    Log_Process_State('Lw_Txntender_Stage');
    ...
    COMMIT;
    /* data need to be written to hist tables */
    Log_Process_State('Lw_Txnrewardredeem_Stage');
    ...
    INSERT INTO Lw_Txnrewardredeem_Stage
      ...
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
                      FROM Lw_Txnrewardredeem_Stage x
                      WHERE t.Rowkey = x.Rowkey)
      AND NOT EXISTS (SELECT 1
                      FROM Ats_Txnheader x
                      WHERE x.a_Txnheaderid = To_Char(t.Txnheaderid));
    ...
    COMMIT;
    /* data need to be written to hist tables */
    Log_Process_State('Lw_Txndetaildiscount_Stage');
    ...
    INSERT INTO Lw_Txndetaildiscount_Stage
      ...
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
                      FROM Lw_Txndetaildiscount_Stage x
                      WHERE t.Rowkey = x.Rowkey)
      AND NOT EXISTS (SELECT 1
                      FROM Ats_Txnheader x
                      WHERE x.a_Txnheaderid = To_Char(t.Txnheaderid));
  ...
  END stage_unprocessed_Tlog01;
  /********************************************************************
  ********************************************************************
  ********************************************************************
  ********************************************************************/
  /******************* Internal function to saves unprocessed txns ***/
  PROCEDURE log_unprocessed_Tlog01 (p_JobNumber NUMBER DEFAULT -1) AS
    PRAGMA AUTONOMOUS_TRANSACTION; /* <---  allow to only commits activity in this local proc */
    /*
      this process will move unprocessed staged data to the unprocessed tables prior to truncating the staging tables.
      This should be called after each sucessful run of the tlog process from the hist_tlog proc, prior to it's actuall operations...
  ...
  BEGIN
    ...
    INSERT INTO Log_Txnrwdrdm_Unprocessed
      ...
      SELECT t.Rowkey
            ...
            ,t.Updatedate
            ,t.Lastdmlid
      FROM Lw_Txnrewardredeem_Stage t
      WHERE 1=1
      AND t.processid = 0
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
    ...
    INSERT INTO Log_Txndtldisc_Unprocessed
      ...
      SELECT t.Rowkey
            ...
            ,t.Updatedate
            ,t.Lastdmlid
      FROM Lw_Txndetaildiscount_Stage t
      WHERE 1=1
      AND t.processid = 0
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
  ...
  BEGIN
   ...
   BEGIN
       ...
       ELSE
        ...
        FROM   LW_LIBJOB lj
        where  lj.jobname = 'DAP-TLog01'
        AND    trunc(lj.starttime) = trunc(v_ProcessDate)
        AND    to_char(lj.starttime, 'mm/dd/yyyy hh:mm:ss') = (select to_char(max(starttime), 'mm/dd/yyyy hh:mm:ss') from lw_libjob where  jobname = 'DAP-TLog01');
       END IF;
      ...
      WHEN Dml_Errors THEN
               ...
               LOOP
                    ...
                    v_Error          := SQLERRM(-sql%BULK_EXCEPTIONS(Indx)
                                                .Error_Code);
                    v_Reason         := 'Failed Procedure Hist_Tlog01: DML_Error ';
                    v_Message        := 'DML_Error';
                    Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                                        p_Envkey    => v_Envkey,
                                        p_Logsource => 'Hist_Tlog01',
                                        p_Filename  => 'Hist_Tlog01',
                                        p_Batchid   => v_Batchid,
                                        p_Jobnumber => v_My_Log_Id,
                                        p_Message   => v_Message,
                                        p_Reason    => v_Reason,
    ...
    THEN
                      v_Messagesfailed := v_Messagesfailed + 1;
                      v_Error          := SQLERRM;
                      v_Reason         := 'DAP Tlog did not process any records';
                      v_Message        := 'DAP Tlog did not process any records';
                      ...
                      Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                                          ...
                                          p_Trycount  => v_Trycount,
                                          p_Msgtime   => SYSDATE);
      Raise_Application_Error(-20001, 'DAP Tlog did not process any records');
    END IF;
...
EXCEPTION
  --AE0-1245: BEGIN Create DML error exception handling for post tlog processing
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog01.Update.Ats_Historytxndetail</proc>' || Chr(10) ||
...
EXCEPTION
  --AE0-1245: BEGIN Create DML error exception handling for post tlog processing
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog01.Insert.Ats_Historytxndetail</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
DECLARE
      CURSOR get_data IS
       ...
       AND t.rowkey in
           (
           SELECT s.rowkey
           From Lw_Txntender_Stage s minus Select x.a_rowkey FROM Ats_Historytxntender x Where x.a_txndate > trunc(sysdate, 'Q')
           );
 TYPE t_tab IS TABLE OF get_data%ROWTYPE; ---<------ creating array definition of cursor rowtype, we'll collect data in chunks
...
BEGIN
      ...
      LOOP
        FETCH get_data BULK COLLECT
          INTO v_tbl2 LIMIT 5000; --<-----  here we say collect 1,000 rows at a time.
        -- Then update address in ATS_MemberDetails table and commit
...
EXCEPTION
  --AE0-1245: BEGIN Create DML error exception handling for post tlog processing
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog01.Insert.Ats_Historytxntender</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
    ...
    INSERT INTO Ats_Historytxnrewardredeem
      ...
      CURSOR get_data IS
       ...
       AND t.rowkey in
           (
           SELECT s.rowkey
           From Lw_Txnrewardredeem_Stage s minus Select x.a_rowkey FROM Ats_Historytxnrewardredeem x Where x.a_txndate > trunc(sysdate, 'Q')
           );
 TYPE t_tab IS TABLE OF get_data%ROWTYPE; ---<------ creating array definition of cursor rowtype, we'll collect data in chunks
...
BEGIN
      ...
      LOOP
        FETCH get_data BULK COLLECT
          INTO v_tbl3 LIMIT 5000; --<-----  here we say collect 1,000 rows at a time.
        -- Then update address in ATS_MemberDetails table and commit
...
EXCEPTION
  --AE0-1245: BEGIN Create DML error exception handling for post tlog processing
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog01.Insert.Ats_Historytxnrewardredeem</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
DECLARE
      CURSOR get_data IS
       ...
       AND t.rowkey in
           (
           SELECT s.rowkey
           From Lw_Txndetaildiscount_Stage s minus Select x.a_rowkey FROM Ats_Historytxndetaildiscount x Where x.a_txndate > trunc(sysdate, 'Q')
           );
 TYPE t_tab IS TABLE OF get_data%ROWTYPE; ---<------ creating array definition of cursor rowtype, we'll collect data in chunks
...
BEGIN
      ...
      LOOP
        FETCH get_data BULK COLLECT
          INTO v_tbl4 LIMIT 5000; --<-----  here we say collect 1,000 rows at a time.
        -- Then update address in ATS_MemberDetails table and commit
...
EXCEPTION
  --AE0-1245: BEGIN Create DML error exception handling for post tlog processing
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog01.Insert.Ats_Historytxndetaildiscount</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
      ...
      WHEN Dml_Errors THEN
               ...
               LOOP
                    ...
                    v_Error          := SQLERRM(-sql%BULK_EXCEPTIONS(Indx)
                                                .Error_Code);
                    v_Reason         := 'Failed Procedure Hist_Tlog01: DML_Error ';
                    v_Message        := 'Dml_Error. Log_Txn_Keys';
                    Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                                        p_Envkey    => v_Envkey,
                                        p_Logsource => 'Hist_Tlog01',
                                        p_Filename  => NULL,
...
--AEO-771 END
  ...
  ********************************************************************
  ********************************************************************/
  PROCEDURE Hist_Tlog01_ByJobNumber(p_Dummy VARCHAR2,
                                    p_JobNumber NUMBER,
                                    Retval  IN OUT Rcursor) AS
    /*
      this process will move staged data to the history tables then truncate the staging tables.
    ...
    */
    v_jobNumber  NUMBER := nvl(p_JobNumber,-1);
  BEGIN
    log_unprocessed_Tlog01 (p_JobNumber => p_JobNumber);
    /* data need to be written to hist tables */
    Log_Process_State('ATS_HISTORYTXNDETAIL');
    ...
    Delete From Lw_Txntender_Stage t Where t.fileid = p_JobNumber;
    ...
    Delete From Lw_Txnrewardredeem_Stage t Where t.fileid = p_JobNumber;
    ...
    Delete From Lw_Txndetaildiscount_Stage t Where t.fileid = p_JobNumber;
    ...
    DELETE FROM Log_Txn_Keys_Stage t
    WHERE t.txnheaderid IN (SELECT txn.txnheaderid FROM Lw_Txndetail_Stage txn WHERE txn.fileid = p_JobNumber);

    Delete From LW_TXNDETAIL_STAGE t Where t.fileid = p_JobNumber;
    commit;

    --commented out this statement because txns can come in late
    --    EXECUTE IMMEDIATE 'truncate tablel LOG_TXN_COUNTS';

  END Hist_Tlog01_ByJobNumber;
  ...
  ********************************************************************/
  ...
  BEGIN
    ...
    COMMIT;
    /* empty out arrays */
    gv_Detail_Tbl     := Type_Tlog01_Stg_Detail_Tbl();
    gv_Tender_Tbl     := Type_Tlog01_Stg_Tender_Tbl();
    gv_Discount_Tbl   := Type_Tlog01_Stg_Discount_Tbl();
    gv_Reward_Tbl     := Type_Tlog01_Stg_Reward_Tbl();
  ...
  ********************************************************************/
  /******************* Internal function to Load  date ************/
  PROCEDURE Load_Stage_Data(p_Head_Rec       IN OUT Type_Tlog01_Stg_Header,
                            p_Detail_Tbl     IN Type_Tlog01_Stg_Detail_Tbl,
                            p_Tender_Tbl     IN Type_Tlog01_Stg_Tender_Tbl,
                            p_Discount_Tbl   IN Type_Tlog01_Stg_Discount_Tbl,
                            p_Reward_Tbl     IN Type_Tlog01_Stg_Reward_Tbl,
...
*/
    ...
    THEN /*National Rollout Changes AEO-1218 begin here        ----------------SCJ*/
      ...
      END IF;
        IF p_txnamount > 1000
      THEN
        p_Head_Rec.Txnqualpurchaseamt := v_qualamount; -- AEO-1847
...
/*National Rollout Changes AEO-1218 end here        ----------------SCJ*/
  ...
  /*National Rollout Changes AEO-1218 here        ----------------SCJ*/
    ...
    */
    IF p_Detail_Tbl.Count > 0 THEN
      FOR i IN 1..p_Detail_Tbl.count LOOP
        ...
        gv_Detail_Tbl(gv_Detail_Tbl.count) :=
                         Type_Tlog01_Stg_Detail(/* rowkey               */p_Detail_Tbl(i).Rowkey,
                                                ...
                                                /* processid            */ CASE WHEN p_Head_Rec.Processid IN (Gv_Status_77child, Gv_Status_77master, Gv_Status_Dup)
                                                 ...
                                                 /*OriginalOrderNumber   */p_Head_Rec.OriginalOrderNumber,
    ...
    ELSIF p_Detail_Tbl.Count = 0 AND v_StoreNumber = 659 AND
      ...
      gv_Detail_Tbl.extend;
      gv_Detail_Tbl(gv_Detail_Tbl.count) :=
                       Type_Tlog01_Stg_Detail(/* rowkey               */ Seq_Rowkey.Nextval,
                                              /* ipcode               */       p_Head_Rec.Ipcode,
                                              /* vckey                */       p_Head_Rec.Vckey,
                                              /* parentrowkey         */       NULL,
                                              /* dtlquantity          */       0,
    ...
    IF p_Discount_Tbl.Count > 0 THEN
      ...
      FOR i IN 1..p_Discount_Tbl.count LOOP
         ...
         gv_Discount_Tbl(gv_Discount_Tbl.count) :=
              Type_Tlog01_Stg_Discount    (/* rowkey         */p_Discount_Tbl(i).Rowkey,
                                           /* ipcode         */p_Head_Rec.Ipcode,
                                           /* vckey          */p_Head_Rec.Vckey,
                                           /* processid      */p_Head_Rec.Processid ,
    ...
    IF p_Reward_Tbl.Count > 0 THEN
      ...
      /* insert redeem records from previous txn */
        ...
        gv_Reward_Tbl(gv_Reward_Tbl.count) :=
                     Type_Tlog01_Stg_Reward( /*rowkey                    */p_Reward_Tbl(i).Rowkey,
                                             /*ipcode                    */p_Head_Rec.Ipcode,
                                             /*vckey                     */p_Head_Rec.Vckey,
                                             /*processid                 */p_Head_Rec.Processid,
    ...
    IF p_Tender_Tbl.Count > 0 THEN
      ...
      /* insert tenders from previous txn */
        ...
        gv_Tender_Tbl(gv_Tender_Tbl.count) :=
                     Type_Tlog01_Stg_Tender( /* rowkey         */p_Tender_Tbl(i).Rowkey,
                                             /* ipcode         */p_Head_Rec.Ipcode,
                                             /* vckey          */p_Head_Rec.Vckey,
                                             /* processid      */p_Head_Rec.Processid,
    ...
    ELSE
      ...
      gv_Tender_Tbl.extend;
      gv_Tender_Tbl(gv_Tender_Tbl.count) :=
                   Type_Tlog01_Stg_Tender( /* rowkey         */Seq_Rowkey.Nextval,
                                           /* ipcode         */p_Head_Rec.Ipcode,
                                           /* vckey          */p_Head_Rec.Vckey,
                                           /* processid      */p_Head_Rec.Processid,
                                           /* storeid        */p_Head_Rec.Txnstoreid,
                                           /* txndate        */p_Head_Rec.Txndate,
                                           /* txnheaderid    */p_Head_Rec.Txnheaderid,
                                           --/* txntenderid    */Tlog01_Id.Nextval,---------------AEO 250 fix ------------------SCJ
  ...
  ********************************************************************/
  ...
  PROCEDURE Stage_Tlog01(p_Filename   VARCHAR2 DEFAULT NULL,
    ...
    */
    /* raw data records */
    v_00_Rec   Type_Tlog01_00header;
    v_01_Rec   Type_Tlog01_01item;
    v_02_Rec   Type_Tlog01_02subtotal;
    v_03_Rec   Type_Tlog01_03salestax;
    v_04_Rec   Type_Tlog01_04tender;
    v_05_Rec   Type_Tlog01_05giftcert;
    v_14_Rec   Type_Tlog01_14foundorder;
    v_15_Rec   Type_Tlog01_15ceditcrdcheck;
    v_18_Rec   Type_Tlog01_18txndiscount;
    v_2733_Rec Type_Tlog01_2733origtxn;
    v_2751_Rec Type_Tlog01_2751discountrsn;
    v_2762_Rec Type_Tlog01_2762valuecard;
    v_2777_Rec Type_Tlog01_2777multidivmsttxn;
    v_2792_Rec Type_Tlog01_2792pomogiftsku;
    v_2793_Rec Type_Tlog01_2793custloyalyt;
    v_2799_Rec Type_Tlog01_2799directordernbr;
    v_99_Rec   Type_Tlog01_99item;    /*AEO-820 changes here -----------------SCJ*/
    /* records and collections used to stage a single txn */
    v_Head_Rec     Type_Tlog01_Stg_Header;
    v_Detail_Rec   Type_Tlog01_Stg_Detail;
    v_Detail_Tbl   Type_Tlog01_Stg_Detail_Tbl := Type_Tlog01_Stg_Detail_Tbl();
    v_Tender_Rec   Type_Tlog01_Stg_Tender;
    v_Tender_Tbl   Type_Tlog01_Stg_Tender_Tbl := Type_Tlog01_Stg_Tender_Tbl();
    v_Discount_Rec Type_Tlog01_Stg_Discount := Type_Tlog01_Stg_Discount(NULL,
                                                                        NULL,
                                                                        NULL,
                                                                        NULL,
                                                                        NULL,
                                                                        ...
                                                                        NULL,
                                                                        NULL);
    v_Discount_Tbl Type_Tlog01_Stg_Discount_Tbl := Type_Tlog01_Stg_Discount_Tbl();
    v_Reward_Rec   Type_Tlog01_Stg_Reward;
    v_Reward_Tbl   Type_Tlog01_Stg_Reward_Tbl := Type_Tlog01_Stg_Reward_Tbl();
    /* misc variables */
      ...
      THEN
        ...
        THEN
          ...
          */
          ...
          v_00_Rec := Get_Tlog01_00header(p_Rec => c.Rec);
        ...
        THEN
          ...
          THEN
            ...
            v_Detail_Tbl          := Type_Tlog01_Stg_Detail_Tbl();
            v_Head_Rec            := Initialize_Head_Rec();
            v_Tender_Tbl          := Type_Tlog01_Stg_Tender_Tbl();
            v_Discount_Tbl        := Type_Tlog01_Stg_Discount_Tbl();
            v_Reward_Tbl          := Type_Tlog01_Stg_Reward_Tbl();
          ...
          THEN
            ...
            v_01_Rec := Get_Tlog01_01item(c.Rec);
...
*/
          ...
          THEN
            /* load rawdata to record type */
            v_99_Rec := Get_Tlog01_99item(c.Rec);
...
*/
          ...
          IF c.Rec_Type = '02'
          THEN
            v_02_Rec             := Get_Tlog01_02subtotal(c.Rec);
            v_Head_Rec.Txnamount := v_02_Rec.Amount;
          END IF;
          /* sales tax  record */
          IF c.Rec_Type = '03'
          THEN
            v_03_Rec                := Get_Tlog01_03salestax(c.Rec);
          ...
          THEN
            ...
            */
            ...
            v_04_Rec := Get_Tlog01_04tender(c.Rec);
          ...
          THEN
            ...
            v_05_Rec := Get_Tlog01_05giftcert(c.Rec);
            ...
            v_Detail_Tbl(v_Detail_Tbl.Count) := v_Detail_Rec;
          END IF;
            v_14_Rec := null; /*National Rollout Changes AEO-1218 begin here        ----------------SCJ*/
          /* foundOrder (in store web order)   record */
          IF c.Rec_Type = '14'
          THEN
            v_14_Rec := Get_Tlog01_14foundorder(c.Rec);
...
/* redesign changes  end here --------------------------------------------------  SCJ  */
          ...
          THEN
            ...
            --v_15_rec :=get_tlog01_15ceditcrdcheck (c.rec);
          END IF;
          /* txn discount string   record */
          IF c.Rec_Type = '18'
          THEN
            /* load raw data to record type */
            v_18_Rec := Get_Tlog01_18txndiscount(c.Rec);
          ...
          THEN
            /* load raw data to record type */
            v_2733_Rec := Get_Tlog01_2733origtxn(c.Rec);
          ...
          THEN
            /* load raw data to record type */
            v_2751_Rec := Get_Tlog01_2751discountrsn(c.Rec);
          ...
          THEN
            /* load raw data to record type */
            v_2762_Rec := Get_Tlog01_2762valuecard(c.Rec);
            v_Detail_Rec              := Initialize_Detail_Rec();
          ...
          THEN
            ...
            v_2777_Rec := Get_Tlog01_2777multidivmsttxn(c.Rec);
            /* load alt info */
          ...
          /*    dont need this, not on tlog spec doc
                  IF c.rec_type = '27' AND c.user_id ='92' THEN
                    v_2792_rec :=get_tlog01_2792pomogiftsku (c.rec);
                    --do something here
                  END IF;
          */
          /* Loyalty ID   record */
          ...
          THEN
            /* load raw data to record type */
            v_2793_Rec              := Get_Tlog01_2793custloyalyt(p_Rec => c.Rec);
            v_Head_Rec.Txnloyaltyid := TRIM(v_2793_Rec.Loyalty_Number);
            /*National Rollout Changes AEO-1218 BEGIN here        ----------------SCJ*/
            v_memberstatus := 1;
             select count(VC.VCKEY) into v_loyaltyidcount from LW_VIRTUALCARD VC  WHERE Vc.Loyaltyidnumber = TRIM(v_Head_Rec.Txnloyaltyid);
            IF (v_loyaltyidcount > 0) THEN
            SELECT Nvl(MAX(Vckey), 0), Nvl(MAX(Vc.Ipcode), 0),NVL(max(LM.MEMBERSTATUS),1)
              INTO v_Head_Rec.Vckey, v_Head_Rec.Ipcode,v_memberstatus
              FROM Lw_Virtualcard Vc
              INNER JOIN LW_LOYALTYMEMBER LM ON LM.IPCODE = VC.IPCODE
                  /* inner join lw_loyaltymember lm on Vc.Ipcode = lm.ipcode*/
             WHERE Vc.Loyaltyidnumber = TRIM(v_Head_Rec.Txnloyaltyid)  group by vc.ipcode;
             ELSE
              v_Head_Rec.Vckey := 0;
              v_Head_Rec.Ipcode := 0;
              v_memberstatus := 1;
          ...
          THEN
            /* load raw data to record type */
            v_2799_Rec := Get_Tlog01_2799directordernbr(c.Rec);
