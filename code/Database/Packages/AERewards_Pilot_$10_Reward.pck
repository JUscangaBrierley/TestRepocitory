CREATE OR REPLACE PACKAGE AERewards_Pilot_$10_Reward AS

     TYPE Rcursor IS REF CURSOR;

     PROCEDURE Issue_Pilot_Reward(p_Typecode VARCHAR2,
     ...
     PROCEDURE Issue_rewards (p_DateIssued IN Bp_Ae.Lw_Memberrewards.Dateissued%TYPE);
								 
     PROCEDURE Consume_Barcodes(p_Dummy VARCHAR2,
                                Retval  IN OUT Rcursor);

END AERewards_Pilot_$10_Reward;
/
CREATE OR REPLACE PACKAGE BODY AERewards_Pilot_$10_Reward IS
...
PROCEDURE Identify_Population IS
     ...
     BEGIN
          DELETE FROM Stg_AeocPilot_Population;
INSERT /*+ append */
          INTO Stg_AeocPilot_Population
SELECT Rownumber,
       Channel,
       Customer_Nbr,
       Loyalty_Number,
       ...
       number_of_active_10_off_reward,
       ...
       timestamp
            FROM (SELECT Rownum AS Rownumber,
                         ...
                         CAST('A92' AS NVARCHAR2(100)) AS Auth_Cd,
                         CAST('' AS NVARCHAR2(100)) AS Suc_Cd,
                         CAST('' AS NVARCHAR2(100)) AS Campaign_Type,
                         CAST('15-OCT-2017' AS NVARCHAR2(100)) AS Campaign_Exp_Date,
                         CAST('305406' AS NVARCHAR2(100)) AS Eid,  ----> Still Needed
                         CAST('' AS NVARCHAR2(100)) AS Campaign_Id,   -----> Still Needed
                         CASE
                         ...
                         Rew.Number_Of_Active_10_Off_Reward,
                         0 as Number_Of_Jeans_Reward,
                         0 AS Number_Bra_Reward,
                  ...
                  /*aeo-614 ADDING SYNCHRONY ACCOUNT KEY LOGIC----------------scj */
                    ...
                    LEFT JOIN (SELECT Mr.Memberid,
                                     SUM(CASE
                                           WHEN r.Name = '$10 AEO Pilot Transition Reward' THEN
                                            1
                                           ELSE
                                            0
                                         END) AS Number_Of_Active_10_Off_Reward
                                FROM Bp_Ae.Lw_Memberrewards Mr, Bp_Ae.Lw_Rewardsdef r
                               WHERE 1 = 1
                                 AND Mr.Rewarddefid = r.Id
                                 AND r.Name IN ('$10 AEO Pilot Transition Reward')
...
PROCEDURE Select_Barcodes(p_Typecode IN Bp_Ae.Ats_Rewardbarcodes.a_Typecode%TYPE) IS
     ...
     BEGIN
          ...
          SELECT COUNT(*) INTO Lv_Rowcount FROM Stg_AeocPilot_Population;
          ...
          THEN
               Raise_Application_Error(-20000,
                                       'Barcode selection did not return enough rows to attach to Pilot population.');
          END IF;
          COMMIT;
END Select_Barcodes;
...
PROCEDURE Backup_Barcode_Assignment IS
          ...
          CURSOR Cur_Rwd IS
               SELECT * FROM Bp_Ae.Ae_Aecc_10reward;
     ...
     BEGIN
          ...
          LOOP
               ...
               FORALL j IN 1 .. v_Tbl.Count SAVE EXCEPTIONS
                    INSERT INTO Ae_Aecc_10reward_Bkp
                         (Channel,
                          Customer_Nbr,
                          Loyalty_Number,
                          Email,
                    ...
                    VALUES
                         (v_Tbl(j).Channel,
                          ...
                          v_Tbl(j).Number_Of_Active_10_Off_Reward,
     ...
     EXCEPTION
          ...
          WHEN OTHERS THEN
               ...
               Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                                   p_Envkey    => v_Envkey,
                                   p_Logsource => 'Issue_Pilot_Reward',
                                   p_Filename  => NULL,
                                   p_Batchid   => v_Batchid,
                                   p_Jobnumber => v_My_Log_Id,
                                   p_Message   => v_Message,
...
PROCEDURE Assign_Barcodes IS
          ...
          CURSOR Cur_Barcd IS
               SELECT T1.Channel,
                      ...
                      T1.Number_Of_Active_10_Off_Reward,
                      ...
                      T1.Timestamp
               FROM   Stg_AeocPilot_Population T1, Stg_Aecc_Barcodes T2
               WHERE  1 = 1
     ...
     BEGIN
          ...
          EXECUTE IMMEDIATE 'truncate table AE_AECC_10REWARD';
               ...
               FORALL i IN 1 .. v_Tbl2.Count SAVE EXCEPTIONS
                    ...
                    VALUES
                         (v_Tbl2(i).Channel,
                          ...
                          v_Tbl2(i).Number_Of_Active_10_Off_Reward,
...
PROCEDURE Issue_Rewards(p_DateIssued IN Bp_Ae.Lw_Memberrewards.Dateissued%TYPE) IS
          ...
          CURSOR Cur_Rwd IS
               ...
               FROM Bp_Ae.Ae_Aecc_10reward rw
     ...
     BEGIN
          ...
          WHERE rd.name = '$10 AEO Pilot Transition Reward';

          SELECT COUNT(*)
          INTO v_prid
          FROM lw_product pr
          WHERE pr.name = 'AEO Connected $10  Reward';

          IF ( v_rwdefid <= 0 ) THEN
             Raise_Application_Error(-20002,
                                       'No reward definition found for lw_rewarddef.name="$10 AEO Pilot Transition Reward"');
          END IF;

           IF ( v_prid <= 0 ) THEN
             Raise_Application_Error(-20002,
                                       'No product definition found for lw_product.name="AEO Connected $10  Reward"');
          END IF;
