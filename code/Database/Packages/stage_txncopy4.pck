CREATE OR REPLACE PACKAGE STAGE_TXNCOPY4 IS
type rcursor IS REF CURSOR;
  /*
PI29741 tlog optimization changes begin here                 -----scj
*/
      PROCEDURE StageToTxnHeadWrk4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailWrk4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnHeader4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetail4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnRedeem4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailDiscount4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTender4(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTables4(p_Processdate VARCHAR2,p_email_list VARCHAR2);
      PROCEDURE TlogInterceptorProcessing4(p_Dummy VARCHAR2,retval IN OUT rcursor);
      PROCEDURE UpdateMemberAttributeFromTlog4(p_Processdate VARCHAR2,retval IN OUT rcursor);
      ...
      PROCEDURE TxnHeaderWrk4Bkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE TxnDetailWrk4Bkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
...
END STAGE_TXNCOPY4;
/
CREATE OR REPLACE PACKAGE BODY STAGE_TXNCOPY4   IS

PROCEDURE StageToTxnHeadWrk4(p_Dummy      VARCHAR2,
...
BEGIN
  ...
  v_jobname   := 'TLog4-TxnHeadWrk4_Staging';
  ...
  BEGIN
    ...
    If v_headerwkcount > 0 then
      Delete from LW_txnheader_Wrk4; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
        ...
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeadWrk4: ';
          ...
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeadWrk4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeadWrk4</proc>' ||
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk4 txncopy4 failure DML ERROR ' ||
                                             sysdate,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeadWrk4</proc>' || Chr(10) ||
      ...
      cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                    ...
                    p_bcc_list      => NULL,
                    p_subject       => v_envkey ||
                                       ' American Eagle Tlog Chunk4 txncopy failure on ' ||
                                       sysdate,
                    p_text_message  => v_message,
                    p_content_type  => 'text/html;charset=UTF8',
                    p_attachments   => v_attachments, --v_attachments,
                    ...
                    p_auth_password => NULL,
                    p_mail_server   => 'cypwebmail.brierleyweb.com');
      Raise_Application_Error(-20002, 'STAGE_TXNCOPY4 job is not finished');

  END;
END StageToTxnHeadWrk4;


PROCEDURE StageToTxnDetailWrk4(p_Dummy      VARCHAR2,
                               p_email_list VARCHAR2,
                               retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TxnDetailWrk4_Staging';
  v_logsource := v_jobname;
  ...
  BEGIN
    ...
    If v_detailitemwkcount > 0 then
      Delete from LW_txndetailitem_Wrk4; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
    OPEN get_data;
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_Wrk4
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailWrk4: ';
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailWrk4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailWrk4</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailWrk4</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailWrk4;

PROCEDURE StageToTxnHeader4(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_Header_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk4 hw
         --AEO-858 BEGIN
         where 1=1
         and not exists
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeader4: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeader4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeader4</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeader4</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
END StageToTxnHeader4;

PROCEDURE StageToTxnDetail4(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_Detailitem_Staging';
  v_logsource := v_jobname;
    ...
    LOOP
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetail4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetail4</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetail4</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetail4;

PROCEDURE StageToTxnRedeem4(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_Redeem_Staging';
  v_logsource := v_jobname;
    ...
    CURSOR get_data IS
      WITH Mem AS
       (select rr.*, hw.header_rowkey
          from LW_TXNREWARDREDEEM_STAGE4 rr
         inner join LW_txnheader_Wrk4 hw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnRedeem4: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnRedeem4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnRedeem4</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnRedeem4</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnRedeem4;

PROCEDURE StageToTxnDetailDiscount4(p_Dummy      VARCHAR2,
                                    p_email_list VARCHAR2,
                                    retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_DetailDiscount_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       from LW_TXNDETAILDISCOUNT_STAGE4 dds
       inner join LW_txndetailitem_Wrk4 dw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailDiscount4: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailDiscount4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailDiscount4</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailDiscount4</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailDiscount4;

PROCEDURE StageToTxnTender4(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_Tender_Staging';
  v_logsource := v_jobname;
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnTender4: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnTender4: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnTender4</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk4 txncopy4 failure DML ERRORS ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnTender4</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnTender4;

PROCEDURE StageToTxnTables4(p_Processdate VARCHAR2,p_email_list VARCHAR2) AS
   p_Dummy2 VARCHAR2(1);
  crsr STAGE_TXNCOPY4.rcursor;
BEGIN

  DECLARE
  BEGIN
   StageToTxnHeadWrk4(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailWrk4(p_Dummy2,p_email_list,crsr);
   Giftcard24HrPurchase(p_Dummy2,crsr);
   BasicReturns24HrAdjust(p_Dummy2,crsr);
   TxnHeaderWrk4Bkp(p_Dummy2,p_email_list,crsr);
   TxnDetailWrk4Bkp(p_Dummy2,p_email_list,crsr);
   StageToTxnHeader4(p_Dummy2,p_email_list,crsr);
   StageToTxnDetail4(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailDiscount4(p_Dummy2,p_email_list,crsr);
   StageToTxnRedeem4(p_Dummy2,p_email_list,crsr);
   StageToTxnTender4(p_Dummy2,p_email_list,crsr);
   --ReturnRedeemTxnPoints(p_Processdate ,p_email_list,crsr);
   END;
END StageToTxnTables4;

PROCEDURE TlogInterceptorProcessing4(p_Dummy VARCHAR2, Retval  IN OUT Rcursor) AS
  ...
  BEGIN
     v_jobname := 'TLog4-TlogInterceptorProcessing';
     v_logsource := v_jobname;
  ...
  utility_pkg.Log_job(P_JOB                => v_my_log_id
         ...
         ,p_jobstatus          => v_jobstatus
         ,p_jobname            => v_jobname);
 utility_pkg.Log_Process_Start(v_jobname, 'TlogInterceptorProcessing4', v_processId);
 /*Logging*/
 DECLARE
   CURSOR get_data IS
      WITH Mem AS
      ...
      from lw_txndetailitem_wrk4 tw
      inner join lw_txnheader_wrk4 hw on hw.txnheaderid = tw.txnheaderid
         ...
         -- For those records whose brands are not set, check each product at detail item
          IF  (bcheckitembrand = 1) then
                   ...
                   If (v_DtlMemberHasBrand = 0) Then
                            ...
                            v_Message        := 'TlogInterceptorProcessing4 is not finished';
...
--  END IF;
    v_Messagesfailed := v_Messagesfailed + 1;
    v_Error          := SQLERRM;
    v_Reason         := 'Failed Procedure TlogInterceptorProcessing4: ';
    v_Message        := '<failed>' || Chr(10) ||
                        '  <details>' || Chr(10) ||
                        '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                        Chr(10) ||
                        '    <proc>TlogInterceptorProcessing4</proc>' ||
                        Chr(10) || '  </details>' ||
                        Chr(10) || '</failed>';
...
END TlogInterceptorProcessing4;

PROCEDURE UpdateMemberAttributeFromTlog4(p_Processdate VARCHAR2, Retval  IN OUT Rcursor) AS
    v_Logsource        VARCHAR2(256) := 'STAGE_TXNCOPY4.UpdateMemberAttributesFromTlog4';
    v_My_Log_Id        NUMBER;
    v_Jobdirection     NUMBER := 0;
    v_Filename         VARCHAR2(512) := 'STAGE_TXNCOPY4.UpdateMemberAttributesFromTlog4';
    v_Starttime        DATE := SYSDATE;
    v_Endtime          DATE;
    v_Messagesreceived NUMBER := 0;
    v_Messagesfailed   NUMBER := 0;
    ...
    v_Jobstatus        NUMBER := 0;
    v_Jobname          VARCHAR2(256) := 'UpdateMemberAttributesFromTlog4';
    v_Batchid          VARCHAR2(256) := 0;
    v_Message          VARCHAR2(256);
    v_Reason           VARCHAR2(256);
    v_Error            VARCHAR2(256);
...
*/
 ...
 /*redesign changes begin end  SCJ  */
  ...
  EXCEPTION
    ...
    RAISE;
  END UpdateMemberAttributeFromTlog4;
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
   -- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog4-Giftcard24HrPurchase';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'Giftcard24HrPurchase', v_processId);
  ...
  BEGIN
    ...
    LOOP
      FETCH get_data BULK COLLECT
        INTO v_tbl LIMIT 10000;
    FORALL j IN 1 .. v_tbl.COUNT
        UPDATE  LW_txndetailitem_Wrk4 DW
...
-- -- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog4-BasicReturns24HrAdjust';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'BasicReturns24HrAdjust', v_processId);
  DECLARE
      ...
      WITH Mem AS
       (
       select hw.txnheaderid from lw_txnheader_wrk4 hw
...
PROCEDURE TxnHeaderWrk4Bkp(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_Header4_Bkp';
  v_logsource := v_jobname;
  ...
  utility_pkg.Log_Process_Start(v_jobname,
                                'BackUp TxnHeaderWrk4',
                                v_processId);
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk4  wk
       --AEO-858 BEGIN
       where 1=1
       and not exists
         (select 1 from LW_txnheader_Wrk4Bkp wb where wb.txnheaderid = wk.txnheaderid) -- Checking for duplicate txn header records
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txnheader_Wrk4Bkp
            (rowkey,
             header_rowkey,
             vckey,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnHeaderWrk4Bkp: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnHeaderWrk4Bkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>TxnHeaderWrk4Bkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk4 txncopy failure DML ERRORS  ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>TxnHeaderWrk4Bkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
END TxnHeaderWrk4Bkp;

PROCEDURE TxnDetailWrk4Bkp(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog4-TXN_Detailitem_Wrk4Bkp';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       --AEO-858 BEGIN
       and not exists
       (select 1 from LW_txndetailitem_Wrk4Bkp db where db.txndetailid = dw.txndetailid) -- Checking for duplicate txn detailitem
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_Wrk4Bkp
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnDetailWrk4Bkp: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnDetailWrk4Bkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY4</pkg>' ||
                              Chr(10) ||
                              '    <proc>TxnDetailWrk4Bkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk4 txncopy failure  DML ERROR ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY4</pkg>' || Chr(10) ||
                   '    <proc>TxnDetailWrk4Bkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog4-ReturnRedeemTxnPoints';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'ReturnRedeemTxnPoints wk4', v_processId);
  DECLARE
...
WHEN OTHERS THEN
           ...
           cio_mail.send(
                ...
                p_cc_list       => NULL,
                p_bcc_list      => NULL,
                p_subject       =>  v_envkey ||' American Eagle Tlog Chunk4 txncopy failure on ' || sysdate  ,
                p_text_message  => v_message,
                p_content_type  => 'text/html;charset=UTF8',
                p_attachments   => v_attachments, --v_attachments,
                p_priority      => '3',
