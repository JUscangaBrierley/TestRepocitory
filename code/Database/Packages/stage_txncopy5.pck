CREATE OR REPLACE PACKAGE STAGE_TXNCOPY5 IS
type rcursor IS REF CURSOR;
  /*
PI29741 tlog optimization changes begin here                 -----scj
*/
      PROCEDURE StageToTxnHeadWrk5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailWrk5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnHeader5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetail5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnRedeem5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailDiscount5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTender5(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTables5(p_Processdate varchar2,p_email_list VARCHAR2);
      PROCEDURE TlogInterceptorProcessing5(p_Dummy VARCHAR2,retval IN OUT rcursor);
      PROCEDURE UpdateMemberAttributeFromTlog5(p_Processdate VARCHAR2,retval IN OUT rcursor);
      ...
      PROCEDURE TxnHeaderWrk5Bkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE TxnDetailWrk5Bkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
...
END STAGE_TXNCOPY5;
/
CREATE OR REPLACE PACKAGE BODY STAGE_TXNCOPY5   IS

PROCEDURE StageToTxnHeadWrk5(p_Dummy      VARCHAR2,
...
BEGIN
  ...
  v_jobname   := 'TLog5-TxnHeadWrk5_Staging';
  ...
  BEGIN
    ...
    If v_headerwkcount > 0 then
      Delete from LW_txnheader_Wrk5; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
    OPEN get_data;
    LOOP
        ...
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeadWrk5: ';
          ...
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeadWrk5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeadWrk5</proc>' ||
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk5 txncopy5 failure DML ERROR ' ||
                                             sysdate,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeadWrk5</proc>' || Chr(10) ||
      ...
      cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                    ...
                    p_bcc_list      => NULL,
                    p_subject       => v_envkey ||
                                       ' American Eagle Tlog Chunk5 txncopy failure on ' ||
                                       sysdate,
                    p_text_message  => v_message,
                    p_content_type  => 'text/html;charset=UTF8',
                    p_attachments   => v_attachments, --v_attachments,
                    ...
                    p_auth_password => NULL,
                    p_mail_server   => 'cypwebmail.brierleyweb.com');
      Raise_Application_Error(-20002, 'STAGE_TXNCOPY5 job is not finished');

  END;
END StageToTxnHeadWrk5;


PROCEDURE StageToTxnDetailWrk5(p_Dummy      VARCHAR2,
                               p_email_list VARCHAR2,
                               retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TxnDetailWrk5_Staging';
  v_logsource := v_jobname;
  ...
  BEGIN
    ...
    If v_detailitemwkcount > 0 then
      Delete from LW_txndetailitem_Wrk5; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
    OPEN get_data;
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_Wrk5
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailWrk5: ';
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailWrk5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailWrk5</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailWrk5</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailWrk5;

PROCEDURE StageToTxnHeader5(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_Header5_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk5 hw
         --AEO-858 BEGIN
         where 1=1
         and not exists
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeader5: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeader5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeader5</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeader5</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnHeader5;

PROCEDURE StageToTxnDetail5(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_Detailitem5_Staging';
  v_logsource := v_jobname;
    ...
    LOOP
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetail5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetail5</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetail5</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetail5;

PROCEDURE StageToTxnRedeem5(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_Redeem5_Staging';
  v_logsource := v_jobname;
    ...
    CURSOR get_data IS
      WITH Mem AS
       (select rr.*, hw.header_rowkey
          from LW_TXNREWARDREDEEM_STAGE5 rr
         inner join LW_txnheader_Wrk5 hw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnRedeem5: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnRedeem5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnRedeem5</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnRedeem5</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnRedeem5;

PROCEDURE StageToTxnDetailDiscount5(p_Dummy      VARCHAR2,
                                    p_email_list VARCHAR2,
                                    retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_DetailDiscount5_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       from LW_TXNDETAILDISCOUNT_STAGE5 dds
       inner join LW_txndetailitem_Wrk5 dw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailDiscount5: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailDiscount5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailDiscount5</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailDiscount5</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailDiscount5;

PROCEDURE StageToTxnTender5(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_Tender5_Staging';
  v_logsource := v_jobname;
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnTender5: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnTender5: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnTender5</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk5 txncopy5 failure DML ERRORS ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnTender5</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnTender5;

PROCEDURE StageToTxnTables5(p_Processdate VARCHAR2,p_email_list VARCHAR2) AS
   p_Dummy2 VARCHAR2(1);
  crsr STAGE_TXNCOPY5.rcursor;
BEGIN

  DECLARE
  BEGIN
   StageToTxnHeadWrk5(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailWrk5(p_Dummy2,p_email_list,crsr);
   Giftcard24HrPurchase(p_Dummy2,crsr);
   BasicReturns24HrAdjust(p_Dummy2,crsr);
   TxnHeaderWrk5Bkp(p_Dummy2,p_email_list,crsr);
   TxnDetailWrk5Bkp(p_Dummy2,p_email_list,crsr);
   StageToTxnHeader5(p_Dummy2,p_email_list,crsr);
   StageToTxnDetail5(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailDiscount5(p_Dummy2,p_email_list,crsr);
   StageToTxnRedeem5(p_Dummy2,p_email_list,crsr);
   StageToTxnTender5(p_Dummy2,p_email_list,crsr);
   --ReturnRedeemTxnPoints(p_Processdate ,p_email_list,crsr);

   END;
END StageToTxnTables5;

PROCEDURE TlogInterceptorProcessing5(p_Dummy VARCHAR2, Retval  IN OUT Rcursor) AS
  ...
  BEGIN
     v_jobname := 'TLog5-TlogInterceptorProcessing';
     v_logsource := v_jobname;
  ...
  utility_pkg.Log_job(P_JOB                => v_my_log_id
         ...
         ,p_jobstatus          => v_jobstatus
         ,p_jobname            => v_jobname);
 utility_pkg.Log_Process_Start(v_jobname, 'TlogInterceptorProcessing5', v_processId);
 /*Logging*/
 DECLARE
   CURSOR get_data IS
      WITH Mem AS
      ...
      from lw_txndetailitem_wrk5 tw
      inner join lw_txnheader_wrk5 hw on hw.txnheaderid = tw.txnheaderid
         ...
         -- For those records whose brands are not set, check each product at detail item
          IF  (bcheckitembrand = 1) then
                   ...
                   If (v_DtlMemberHasBrand = 0) Then
                            ...
                            v_Message        := 'TlogInterceptorProcessing5 is not finished';
...
--  END IF;
    v_Messagesfailed := v_Messagesfailed + 1;
    v_Error          := SQLERRM;
    v_Reason         := 'Failed Procedure TlogInterceptorProcessing5: ';
    v_Message        := '<failed>' || Chr(10) ||
                        '  <details>' || Chr(10) ||
                        '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                        Chr(10) ||
                        '    <proc>TlogInterceptorProcessing5</proc>' ||
                        Chr(10) || '  </details>' ||
                        Chr(10) || '</failed>';
...
END TlogInterceptorProcessing5;

PROCEDURE UpdateMemberAttributeFromTlog5(p_Processdate VARCHAR2, Retval  IN OUT Rcursor) AS
    v_Logsource        VARCHAR2(256) := 'STAGE_TXNCOPY5.UpdateMemberAttributesFromTlog5';
    v_My_Log_Id        NUMBER;
    v_Jobdirection     NUMBER := 0;
    v_Filename         VARCHAR2(512) := 'STAGE_TXNCOPY5.UpdateMemberAttributesFromTlog5';
    v_Starttime        DATE := SYSDATE;
    v_Endtime          DATE;
    v_Messagesreceived NUMBER := 0;
    v_Messagesfailed   NUMBER := 0;
    ...
    v_Jobstatus        NUMBER := 0;
    v_Jobname          VARCHAR2(256) := 'UpdateMemberAttributesFromTlog5';
    v_Batchid          VARCHAR2(256) := 0;
    v_Message          VARCHAR2(256);
    v_Reason           VARCHAR2(256);
    v_Error            VARCHAR2(256);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                          ...
                          p_Msgtime   => SYSDATE);
  RAISE;
  END UpdateMemberAttributeFromTlog5;
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
   -- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog5-Giftcard24HrPurchase';
     v_logsource := v_jobname;
  ...
  BEGIN
    ...
    LOOP
      FETCH get_data BULK COLLECT
        INTO v_tbl LIMIT 10000;
    FORALL j IN 1 .. v_tbl.COUNT -- SAVE EXCEPTIONS
        UPDATE  LW_txndetailitem_Wrk5 DW
...
-- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog5-BasicReturns24HrAdjust';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'BasicReturns24HrAdjust', v_processId);
  DECLARE
      ...
      WITH Mem AS
       (
       select hw.txnheaderid from lw_txnheader_wrk5 hw
...
PROCEDURE TxnHeaderWrk5Bkp(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_Header5_Bkp';
  v_logsource := v_jobname;
  ...
  utility_pkg.Log_Process_Start(v_jobname,
                                'BackUp TxnHeaderWrk5',
                                v_processId);
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk5 wk
       --AEO-858 BEGIN
       where 1=1
       and not exists
         (select 1 from LW_txnheader_Wrk5Bkp wb where wb.txnheaderid = wk.txnheaderid) -- Checking for duplicate txn header records
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txnheader_Wrk5Bkp
            (rowkey,
             header_rowkey,
             vckey,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnHeaderWrk5Bkp: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnHeaderWrk5Bkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>TxnHeaderWrk5Bkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk5 txncopy failure DML ERRORS ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>TxnHeaderWrk5Bkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
END TxnHeaderWrk5Bkp;

PROCEDURE TxnDetailWrk5Bkp(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog5-TXN_Detailitem_Wrk5Bkp';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       --AEO-858 BEGIN
       and not exists
       (select 1 from LW_txndetailitem_Wrk5Bkp db where db.txndetailid = dw.txndetailid) -- Checking for duplicate txn detailitem
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_Wrk5Bkp
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnDetailWrk5Bkp: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnDetailWrk5Bkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY5</pkg>' ||
                              Chr(10) ||
                              '    <proc>TxnDetailWrk5Bkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk5 txncopy failure DML ERROR ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY5</pkg>' || Chr(10) ||
                   '    <proc>TxnDetailWrk5Bkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog5-ReturnRedeemTxnPoints';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'ReturnRedeemTxnPoints wk5', v_processId);
  DECLARE
...
WHEN OTHERS THEN
           ...
           cio_mail.send(
                ...
                p_cc_list       => NULL,
                p_bcc_list      => NULL,
                p_subject       =>  v_envkey ||' American Eagle Tlog Chunk5 txncopy failure on ' || sysdate  ,
                p_text_message  => v_message,
                p_content_type  => 'text/html;charset=UTF8',
                p_attachments   => v_attachments, --v_attachments,
                p_priority      => '3',
