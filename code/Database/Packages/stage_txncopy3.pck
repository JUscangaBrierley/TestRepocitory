CREATE OR REPLACE PACKAGE STAGE_TXNCOPY3 IS
type rcursor IS REF CURSOR;
  /*
PI29741 tlog optimization changes begin here                 -----scj
*/
      PROCEDURE StageToTxnHeadWrk3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailWrk3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnHeader3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetail3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnRedeem3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnDetailDiscount3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTender3(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE StageToTxnTables3(p_Processdate VARCHAR2,p_email_list VARCHAR2);
      PROCEDURE TlogInterceptorProcessing3(p_Dummy VARCHAR2,retval IN OUT rcursor);
      PROCEDURE UpdateMemberAttributeFromTlog3(p_Processdate VARCHAR2,retval IN OUT rcursor);
      ...
      PROCEDURE TxnHeaderWrk3Bkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
      PROCEDURE TxnDetailWrk3Bkp(p_Dummy VARCHAR2,p_email_list VARCHAR2,retval IN OUT rcursor);
...
END STAGE_TXNCOPY3;
/
CREATE OR REPLACE PACKAGE BODY STAGE_TXNCOPY3   IS

PROCEDURE StageToTxnHeadWrk3(p_Dummy      VARCHAR2,
...
BEGIN
  ...
  v_jobname   := 'TLog3-TxnHeadWrk3_Staging';
  ...
  BEGIN
    ...
    If v_headerwkcount > 0 then
      Delete from LW_txnheader_Wrk3; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
    OPEN get_data;
    LOOP
        ...
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeadWrk3: ';
          ...
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeadWrk3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeadWrk3</proc>' ||
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk3 txncopy3 failure DML ERROR ' ||
                                             sysdate,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeadWrk3</proc>' || Chr(10) ||
      ...
      cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                    ...
                    p_bcc_list      => NULL,
                    p_subject       => v_envkey ||
                                       ' American Eagle Tlog Chunk3 txncopy failure on ' ||
                                       sysdate,
                    p_text_message  => v_message,
                    p_content_type  => 'text/html;charset=UTF8',
                    p_attachments   => v_attachments, --v_attachments,
                    ...
                    p_auth_password => NULL,
                    p_mail_server   => 'cypwebmail.brierleyweb.com');
      Raise_Application_Error(-20002, 'STAGE_TXNCOPY3 job is not finished');

  END;
END StageToTxnHeadWrk3;


PROCEDURE StageToTxnDetailWrk3(p_Dummy      VARCHAR2,
                               p_email_list VARCHAR2,
                               retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TxnDetailWrk3_Staging';
  v_logsource := v_jobname;
  ...
  BEGIN
    ...
    If v_detailitemwkcount > 0 then
      Delete from LW_txndetailitem_Wrk3; -- not using truncate because this causes ORA-08103 in the header rules processing
      commit;
    End if;
    OPEN get_data;
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_Wrk3
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailWrk2\3: ';
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailWrk3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailWrk3</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailWrk3</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailWrk3;

PROCEDURE StageToTxnHeader3(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_Header_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk3 hw
         --AEO-858 BEGIN
         where 1=1
         and not exists
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnHeader3: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnHeader3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnHeader3</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnHeader3</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnHeader3;

PROCEDURE StageToTxnDetail3(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_Detailitem_Staging';
  v_logsource := v_jobname;
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailWrk3: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetail3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetail3</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetail3</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetail3;

PROCEDURE StageToTxnRedeem3(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_Redeem_Staging';
  v_logsource := v_jobname;
    ...
    CURSOR get_data IS
      WITH Mem AS
       (select rr.*, hw.header_rowkey
          from LW_TXNREWARDREDEEM_STAGE3 rr
         inner join LW_txnheader_Wrk3 hw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnRedeem3: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnRedeem3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnRedeem3</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnRedeem3</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnRedeem3;

PROCEDURE StageToTxnDetailDiscount3(p_Dummy      VARCHAR2,
                                    p_email_list VARCHAR2,
                                    retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_DetailDiscount_Staging';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       from LW_TXNDETAILDISCOUNT_STAGE3 dds
       inner join LW_txndetailitem_Wrk3 dw
      ...
      EXCEPTION
        WHEN dml_errors THEN
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnDetailDiscount3: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnDetailDiscount3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnDetailDiscount3</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnDetailDiscount3</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnDetailDiscount3;

PROCEDURE StageToTxnTender3(p_Dummy      VARCHAR2,
                            p_email_list VARCHAR2,
                            retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_Tender_Staging';
  v_logsource := v_jobname;
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure StageToTxnTender3: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure StageToTxnTender3: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>StageToTxnTender3</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk3 txncopy3 failure DML ERRORS  ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>StageToTxnTender3</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
  ...
  END;
END StageToTxnTender3;

PROCEDURE StageToTxnTables3(p_Processdate VARCHAR2,p_email_list VARCHAR2) AS
   p_Dummy2 VARCHAR2(1);
  crsr STAGE_TXNCOPY3.rcursor;
BEGIN

  DECLARE
  BEGIN
   StageToTxnHeadWrk3(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailWrk3(p_Dummy2,p_email_list,crsr);
   Giftcard24HrPurchase(p_Dummy2,crsr);
   BasicReturns24HrAdjust(p_Dummy2,crsr);
   TxnHeaderWrk3Bkp(p_Dummy2,p_email_list,crsr);
   TxnDetailWrk3Bkp(p_Dummy2,p_email_list,crsr);
   StageToTxnHeader3(p_Dummy2,p_email_list,crsr);
   StageToTxnDetail3(p_Dummy2,p_email_list,crsr);
   StageToTxnDetailDiscount3(p_Dummy2,p_email_list,crsr);
   StageToTxnRedeem3(p_Dummy2,p_email_list,crsr);
   StageToTxnTender3(p_Dummy2,p_email_list,crsr);
   --ReturnRedeemTxnPoints(p_Processdate ,p_email_list,crsr);
   END;
END StageToTxnTables3;

PROCEDURE TlogInterceptorProcessing3(p_Dummy VARCHAR2, Retval  IN OUT Rcursor) AS
  ...
  BEGIN
     v_jobname := 'TLog3-TlogInterceptorProcessing';
     v_logsource := v_jobname;
  ...
  utility_pkg.Log_job(P_JOB                => v_my_log_id
         ...
         ,p_jobstatus          => v_jobstatus
         ,p_jobname            => v_jobname);
 utility_pkg.Log_Process_Start(v_jobname, 'TlogInterceptorProcessing3', v_processId);
 /*Logging*/
 DECLARE
   CURSOR get_data IS
      WITH Mem AS
      ...
      from lw_txndetailitem_wrk3 tw
      inner join lw_txnheader_wrk3 hw on hw.txnheaderid = tw.txnheaderid
         ...
         -- For those records whose brands are not set, check each product at detail item
          IF  (bcheckitembrand = 1) then
                   ...
                   If (v_DtlMemberHasBrand = 0) Then
                            ...
                            v_Message        := 'TlogInterceptorProcessing3 is not finished';
...
--  END IF;
    v_Messagesfailed := v_Messagesfailed + 1;
    v_Error          := SQLERRM;
    v_Reason         := 'Failed Procedure TlogInterceptorProcessing3: ';
    v_Message        := '<failed>' || Chr(10) ||
                        '  <details>' || Chr(10) ||
                        '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                        Chr(10) ||
                        '    <proc>TlogInterceptorProcessing3</proc>' ||
                        Chr(10) || '  </details>' ||
                        Chr(10) || '</failed>';
...
END TlogInterceptorProcessing3;

PROCEDURE UpdateMemberAttributeFromTlog3(p_Processdate VARCHAR2, Retval  IN OUT Rcursor) AS
    v_Logsource        VARCHAR2(256) := 'STAGE_TXNCOPY3.UpdateMemberAttributesFromTlog3';
    v_My_Log_Id        NUMBER;
    v_Jobdirection     NUMBER := 0;
    v_Filename         VARCHAR2(512) := 'STAGE_TXNCOPY3.UpdateMemberAttributesFromTlog3';
    v_Starttime        DATE := SYSDATE;
    v_Endtime          DATE;
    v_Messagesreceived NUMBER := 0;
    v_Messagesfailed   NUMBER := 0;
    ...
    v_Jobstatus        NUMBER := 0;
    v_Jobname          VARCHAR2(256) := 'UpdateMemberAttributesFromTlog3';
    v_Batchid          VARCHAR2(256) := 0;
    v_Message          VARCHAR2(256);
    v_Reason           VARCHAR2(256);
    v_Error            VARCHAR2(256);
...
*/
   ...
   /* DECLARE
    ...
    BEGIN
      ...
      LOOP
        ...
        FORALL i IN 1 .. v_tbl.count -- SAVE EXCEPTIONS --<------ here we update all 1,000 rows at once, limiting the statement i/o by 9999 calls per loop
          UPDATE ats_memberdetails mds
             SET  mds.a_Netspend = Nvl(MDS.a_Netspend,0) +  Nvl(v_tbl(i).txnqualpurchaseamt,0)
           WHERE mds.a_ipcode = v_tbl(i).ipcode;
 ...
 /*redesign changes begin end  SCJ  */
  ...
  EXCEPTION
    ...
    RAISE;
  END UpdateMemberAttributeFromTlog3;
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
   -- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog3-Giftcard24HrPurchase';
     v_logsource := v_jobname;
  ...
  BEGIN
    ...
    LOOP
      FETCH get_data BULK COLLECT
        INTO v_tbl LIMIT 10000;
    FORALL j IN 1 .. v_tbl.COUNT -- SAVE EXCEPTIONS
        UPDATE  LW_txndetailitem_Wrk3 DW
...
-- v_dap_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog3-BasicReturns24HrAdjust';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'BasicReturns24HrAdjust', v_processId);
  DECLARE
      ...
      WITH Mem AS
       (
       select hw.txnheaderid from lw_txnheader_wrk3 hw
...
PROCEDURE TxnHeaderWrk3Bkp(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_Header3_Bkp';
  v_logsource := v_jobname;
  ...
  utility_pkg.Log_Process_Start(v_jobname,
                                'BackUp TxnHeaderWrk3',
                                v_processId);
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       (
       select * from LW_txnheader_Wrk3 wk
       --AEO-858 BEGIN
       where 1=1
       and not exists
         (select 1 from LW_txnheader_Wrk3Bkp wb where wb.txnheaderid = wk.txnheaderid) -- Checking for duplicate txn header records
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txnheader_Wrk3Bkp
            (rowkey,
             header_rowkey,
             vckey,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnHeaderWrk3Bkp: ';
            v_Message        := 'vckey: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .vckey || 'txnheaderid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txnheaderid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnHeaderWrk3Bkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>TxnHeaderWrk3Bkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk3 txncopy failure DML ERRORS ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>TxnHeaderWrk3Bkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
END TxnHeaderWrk3Bkp;

PROCEDURE TxnDetailWrk3Bkp(p_Dummy      VARCHAR2,
                           p_email_list VARCHAR2,
                           retval       IN OUT rcursor) AS
  v_attachments cio_mail.attachment_tbl_type;
  dml_errors EXCEPTION;
...
BEGIN
  ...
  v_jobname   := 'TLog3-TXN_Detailitem_Wrk3Bkp';
  v_logsource := v_jobname;
  ...
  DECLARE
    CURSOR get_data IS
      WITH Mem AS
       ...
       --AEO-858 BEGIN
       and not exists
       (select 1 from LW_txndetailitem_Wrk3Bkp db where db.txndetailid = dw.txndetailid) -- Checking for duplicate txn detailitem
       --AEO-858 END
       )
      SELECT * FROM Mem;
    TYPE t_tab IS TABLE OF get_data%ROWTYPE;
  ...
  BEGIN
    ...
    LOOP
      ...
      Begin
        FORALL j IN 1 .. v_tbl.COUNT SAVE EXCEPTIONS
          Insert into LW_txndetailitem_Wrk3Bkp
            (rowkey,
             dtl_rowkey,
             ipcode,
             parentrowkey,
      ...
      EXCEPTION
        WHEN dml_errors THEN
          ...
          FOR indx IN 1 .. sql%BULK_EXCEPTIONS.COUNT LOOP
            ...
            v_Error          := SQLERRM(-SQL%BULK_EXCEPTIONS(indx)
                                        .ERROR_CODE);
            v_Reason         := 'Failed Records in Procedure TxnDetailWrk2Bkp: ';
            v_Message        := 'ipcode: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .ipcode || 'txndetailid: ' || v_tbl(sql%BULK_EXCEPTIONS(indx).ERROR_INDEX)
                               .txndetailid;
          ...
          v_Messagesfailed := v_Messagesfailed;
          v_Error          := SQLERRM;
          v_Reason         := 'Failed Procedure TxnDetailWrk3Bkp: ';
          v_Message        := '<failed>' || Chr(10) || '  <details>' ||
                              Chr(10) || '    <pkg>STAGE_TXNCOPY3</pkg>' ||
                              Chr(10) ||
                              '    <proc>TxnDetailWrk3Bkp</proc>' ||
                              Chr(10) || '  </details>' || Chr(10) ||
                              '</failed>';
          ...
          if v_email_flag = 0 then
            cio_mail.send(p_from_email    => 'AEJobs@brierley.com',
                          ...
                          p_bcc_list      => NULL,
                          p_subject       => v_envkey ||
                                             ' American Eagle Tlog Chunk3 txncopy failure DML ERROR ' ||
                                             sysdate,
                          p_text_message  => v_message,
                          p_content_type  => 'text/html;charset=UTF8',
                          p_attachments   => v_attachments, --v_attachments,
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TXNCOPY3</pkg>' || Chr(10) ||
                   '    <proc>TxnDetailWrk3Bkp</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
BEGIN
  v_my_log_id := utility_pkg.get_LIBJobID();
     ...
     v_jobname := 'TLog3-ReturnRedeemTxnPoints';
     v_logsource := v_jobname;
...
utility_pkg.Log_Process_Start(v_jobname, 'ReturnRedeemTxnPoints wk3', v_processId);
  DECLARE
...
WHEN OTHERS THEN
           ...
           cio_mail.send(
                ...
                p_cc_list       => NULL,
                p_bcc_list      => NULL,
                p_subject       =>  v_envkey ||' American Eagle Tlog Chunk3 txncopy failure on ' || sysdate  ,
                p_text_message  => v_message,
                p_content_type  => 'text/html;charset=UTF8',
                p_attachments   => v_attachments, --v_attachments,
                p_priority      => '3',
