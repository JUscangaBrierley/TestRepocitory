CREATE OR REPLACE PACKAGE Stage_Tlog4 IS

  /*
      usage

      Stage_Tlog04 is the main process
         It will run hist_Tlog04 for you.
         Hist_Tlog04 is called prior to processing... so only the privious data is pushed to hist.
  ...
  */
  ...
  Gv_Process       CONSTANT VARCHAR2(256) := Lower(TRIM('stage_Tlog04'));
  ...
  PROCEDURE Stage_Tlog04(p_Filename   VARCHAR2 DEFAULT NULL,
                         p_Test_Count NUMBER DEFAULT 0,
                         Retval       IN OUT Rcursor);

  PROCEDURE Hist_Tlog04(p_Dummy VARCHAR2, p_Process_Date Date,
                        Retval  IN OUT Rcursor);

  PROCEDURE Hist_Tlog04_ByJobNumber(p_Dummy VARCHAR2, p_JobNumber NUMBER,
                        Retval  IN OUT Rcursor);

  PROCEDURE Clear_Stage(p_Dummy VARCHAR2,
                        Retval  IN OUT Rcursor);

  PROCEDURE Stage_unprocessed_Tlog04 (p_Dummy VARCHAR2,
                                      Retval       IN OUT Rcursor);
  PROCEDURE Re_ProcessStagedTlog(p_Dummy VARCHAR2,
                                 Retval  IN OUT Rcursor);

  PROCEDURE log_unprocessed_Tlog04 (p_JobNumber NUMBER DEFAULT -1); /* this called by hist_tlog, so dont run manually*/
  ...
  CURSOR Get_Discounts(p_Header_Id  VARCHAR2,
    ...
    SELECT Rowkey,
           ...
           Lastdmlid,
           Fileid
      FROM Lw_Txndetaildiscount_Stage4
  ...
  CURSOR Get_Rewards(p_Header_Id  VARCHAR2,
    ...
    SELECT Rowkey,
           ...
           Updatedate,
           Lastdmlid
      FROM Lw_Txnrewardredeem_Stage4
     WHERE Txnheaderid = p_Header_Id
       AND p_Source IN ('stage', 'hist')
       AND Processid = p_Process_Id
    UNION ALL
...
END Stage_Tlog4;
/
CREATE OR REPLACE PACKAGE BODY Stage_Tlog4 IS
   gv_Detail_Tbl     Type_Tlog04_Stg_Detail_Tbl := Type_Tlog04_Stg_Detail_Tbl();
   gv_Tender_Tbl     Type_Tlog04_Stg_Tender_Tbl := Type_Tlog04_Stg_Tender_Tbl();
   gv_Discount_Tbl   Type_Tlog04_Stg_Discount_Tbl := Type_Tlog04_Stg_Discount_Tbl();
   gv_Reward_Tbl     Type_Tlog04_Stg_Reward_Tbl  := Type_Tlog04_Stg_Reward_Tbl();
...
FUNCTION is_duplicated(p_txnheaderid  number) RETURN number IS
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Get_Tlog04_00header(p_Rec IN VARCHAR2) RETURN Type_Tlog04_00header IS
    v_Return_Rec Type_Tlog04_00header;
    /*
  ...
  BEGIN
    IF p_Rec LIKE '00%'
    THEN
      v_Return_Rec := Type_Tlog04_00header(Substr(p_Rec, 001, 2),
                                           Substr(p_Rec, 003, 5),
                                           Substr(p_Rec, 008, 3),
                                           Substr(p_Rec, 011, 9),
                                           Substr(p_Rec, 020, 6),
  ...
  ********************************************************************/
    ...
    FUNCTION Get_Tlog04_01item(p_Rec IN VARCHAR2) RETURN Type_Tlog04_01item IS
    v_Return_Rec    Type_Tlog04_01item;
...
-- PI 31647 - 77kids class codes reassigned to AE - points not added - Start
    ...
    IF p_Rec LIKE '01%'
    THEN
      v_Return_Rec := Type_Tlog04_01item(Substr(p_Rec, 001, 2),
                                         Substr(p_Rec, 003, 1),
                                         Substr(p_Rec, 004, 9),
  ...
  ********************************************************************/
    ...
    FUNCTION Get_Tlog04_99item(p_Rec IN VARCHAR2) RETURN Type_Tlog04_99item IS
    v_Return_Rec    Type_Tlog04_99item;
    v_Actual_Amount NUMBER;
    v_Retail_Amount NUMBER;
    v_Quantity      NUMBER;
    v_77Kids_ClassCode NUMBER;
  ...
  BEGIN
    ...
    IF p_Rec LIKE '99%'
    THEN
      v_Return_Rec := Type_Tlog04_99item(Substr(p_Rec, 001, 2),
                                         Substr(p_Rec, 003, 1),
                                         Substr(p_Rec, 004, 9),
                                         Ltrim(TRIM(Substr(p_Rec, 013, 12)),
                                               '0') /* sku */,
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_02subtotal(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_02subtotal IS
    v_Return_Rec Type_Tlog04_02subtotal;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog04_02subtotal(Substr(p_Rec, 001, 2),
                                           Substr(p_Rec, 003, 1),
                                           v_Amount);
    /*  tlog record spec
    (STRING_TYPE               varchar2(2)   \*001 - 002*\
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_03salestax(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_03salestax IS
    v_Return_Rec Type_Tlog04_03salestax;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog04_03salestax(Substr(p_Rec, 001, 1),
                                           Substr(p_Rec, 003, 1),
                                           Substr(p_Rec, 004, 1),
                                           v_Amount);
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_04tender(p_Rec IN VARCHAR2) RETURN Type_Tlog04_04tender IS
    v_Return_Rec Type_Tlog04_04tender;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog04_04tender(Substr(p_Rec, 001, 2),
                                         Substr(p_Rec, 003, 2),
                                         Substr(p_Rec, 005, 2),
                                         v_Amount);
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_05giftcert(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_05giftcert IS
    v_Return_Rec Type_Tlog04_05giftcert;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog04_05giftcert(Substr(p_Rec, 001, 2),
                                           Substr(p_Rec, 003, 1),
                                           Substr(p_Rec, 014, 9),
                                           Substr(p_Rec, 013, 5),
                                           v_Amount);
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_14foundorder(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_14foundorder IS
    v_Return_Rec Type_Tlog04_14foundorder;
    v_Amount     NUMBER;
    v_Taxrate    NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
  ...
  BEGIN
   ...
   -- v_Taxrate    := To_Number(Substr(p_Rec, 012, 5)) * .001;  /*(Commented to avoid 10/26 tlog issue ,if tax is double digit,then exceeds column length - char2(5)) */
    v_Return_Rec := Type_Tlog04_14foundorder(Substr(p_Rec, 001, 2),
                                             Substr(p_Rec, 003, 1),
                                             Substr(p_Rec, 004, 1),
                                             Substr(p_Rec, 005, 1),
                                             Substr(p_Rec, 006, 1),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_15ceditcrdcheck(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_15ceditcrdcheck IS
    v_Return_Rec Type_Tlog04_15ceditcrdcheck;
    v_Amount     NUMBER;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    ...
    v_Return_Rec := Type_Tlog04_15ceditcrdcheck(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 2),
                                                Substr(p_Rec, 005, 27),
                                                Substr(p_Rec, 032, 4),
                                                Substr(p_Rec, 036, 4),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_18txndiscount(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_18txndiscount IS
    v_Return_Rec Type_Tlog04_18txndiscount;
    /*
      Parses tlog rec and loads into formatted array.
    */
    v_Amount1 NUMBER;
  ...
  BEGIN
    ...
    v_Return_Rec := Type_Tlog04_18txndiscount(Substr(p_Rec, 001, 2),
                                              Substr(p_Rec, 003, 1),
                                              Substr(p_Rec, 004, 1),
                                              v_Amount1,
                                              v_Amount2);
    ...
    */
    ...
    RETURN v_Return_Rec;
  END Get_Tlog04_18txndiscount;
  /********************************************************************
  ********************************************************************
  ********************************************************************
  ********************************************************************/

  FUNCTION Get_Tlog04_2733origtxn(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2733origtxn IS
    v_Return_Rec Type_Tlog04_2733origtxn;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog04_2733origtxn(Substr(p_Rec, 001, 2),
                                            Substr(p_Rec, 003, 3),
                                            Substr(p_Rec, 006, 1),
                                            Substr(p_Rec, 007, 2),
                                            Substr(p_Rec, 009, 6),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_2751discountrsn(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2751discountrsn IS
    v_Return_Rec Type_Tlog04_2751discountrsn;
    /*
      Parses tlog rec and loads into formatted array.
    */
  ...
  BEGIN
    ...
    /*
    v_Return_Rec := Type_Tlog04_2751discountrsn(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 3),
                                                Substr(p_Rec, 006, 1),
                                                Substr(p_Rec, 007, 2),
                                                Substr(p_Rec, 009, 1),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_2762valuecard(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2762valuecard IS
    v_Return_Rec Type_Tlog04_2762valuecard;
    /*
      Parses tlog rec and loads into formatted array.
    */
  ...
  BEGIN
    ...
    v_Return_Rec  := Type_Tlog04_2762valuecard(Substr(p_Rec, 001, 2),
                                               Substr(p_Rec, 003, 3),
                                               Substr(p_Rec, 006, 1),
                                               Substr(p_Rec, 007, 2),
                                               Substr(p_Rec, 009, 2),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_2777multidivmsttxn(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2777multidivmsttxn IS
    v_Return_Rec Type_Tlog04_2777multidivmsttxn;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog04_2777multidivmsttxn(Substr(p_Rec, 001, 2),
                                                   Substr(p_Rec, 003, 3),
                                                   Substr(p_Rec, 006, 1),
                                                   Substr(p_Rec, 007, 2),
                                                   Substr(p_Rec, 009, 5),
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_2792pomogiftsku(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2792pomogiftsku IS
    v_Return_Rec Type_Tlog04_2792pomogiftsku;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog04_2792pomogiftsku(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 3),
                                                Substr(p_Rec, 006, 1),
                                                Substr(p_Rec, 007, 2),
                                                Substr(p_Rec, 009, 12));
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_2793custloyalyt(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2793custloyalyt IS
    v_Return_Rec Type_Tlog04_2793custloyalyt;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog04_2793custloyalyt(Substr(p_Rec, 001, 2),
                                                Substr(p_Rec, 003, 3),
                                                Substr(p_Rec, 006, 1),
                                                Substr(p_Rec, 007, 2),
                                                Substr(p_Rec, 009, 14));
  ...
  ********************************************************************/
  ...
  FUNCTION Get_Tlog04_2799directordernbr(p_Rec IN VARCHAR2)
    RETURN Type_Tlog04_2799directordernbr IS
    v_Return_Rec Type_Tlog04_2799directordernbr;
    /*
      Parses tlog rec and loads into formatted array.
    */
  BEGIN
    v_Return_Rec := Type_Tlog04_2799directordernbr(Substr(p_Rec, 001, 2),
                                                   Substr(p_Rec, 003, 3),
                                                   Substr(p_Rec, 006, 1),
                                                   Substr(p_Rec, 007, 2),
 ...
 /********************************************************************
  ...
  Stage_Tlog04 procedure that will go back through and zero out any
  ...
  ********************************************************************/
  ...
  BEGIN
    ...
    DECLARE
      ...
      from lw_txndetail_stage4 stg
      Where  1=1
      and    stg.txntypeid = 2
      And    stg.txnoriginaltxnrowkey is null
      and  stg.ordernumber is null
      and    not exists (select 1 from lw_txndetail_stage4 stg3 where stg3.txnstoreid = stg.txnoriginalstoreid and trunc(stg3.txndate) = stg.txnoriginaltxndate and stg3.txnnumber = stg.txnoriginaltxnnumber and stg3.ipcode > 0)
      union
            select distinct stg.txnheaderid
      from lw_txndetail_stage4 stg
      Where  1=1
      ...
      and    not exists (select 1 from lw_txndetail_stage4 stg3 where stg3.txnstoreid = stg.txnoriginalstoreid and stg3.txnnumber = stg.txnoriginaltxnnumber and stg3.ipcode > 0)
      ;
    ...
    BEGIN
      ...
      LOOP
        ...
        FORALL i IN 1 .. v_tbl.count --<------ here we update all 1,000 rows at once, limiting the statement i/o by 9999 calls per loop
          UPDATE lw_txndetail_stage4 stg
...
*/
  BEGIN
    ...
    DECLARE
      CURSOR get_data2 IS
        SELECT certificatecode, txndate from lw_txnrewardredeem_stage4;
...
/*Redesign Changes begin Here ---------------------------------------------SCJ*/
  ...
  ********************************************************************/
  PROCEDURE UpdateMemberAttributesFromTlog AS
    v_Logsource        VARCHAR2(256) := 'Stage_Tlog4.UpdateMemberAttributesFromTlog';
    v_My_Log_Id        NUMBER;
    v_Jobdirection     NUMBER := 0;
    v_Filename         VARCHAR2(512) := 'Stage_Tlog4.UpdateMemberAttributesFromTlog';
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      v_Error          := SQLERRM;
      v_Reason         := 'Failed Procedure UpdateMemberAttributesFromTlog4';
      v_Message        := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                          '    <pkg>Stage_Tlog4</pkg>' || Chr(10) ||
                          '    <proc>UpdateMemberAttributesFromTlog4</proc>' ||
  ...
  ********************************************************************/
  ...
  BEGIN
    ...
    */
    v_Sql := 'ALTER TABLE ext_Tlog04' || Chr(10) || 'LOCATION (AE_IN' ||
             Chr(58) || '''' || p_Filename || ''')';
    EXECUTE IMMEDIATE v_Sql;
    Clear_Infile('ext_Tlog04_log');
  END Initialize_Tbl;
  /********************************************************************
  ********************************************************************
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Head_Rec RETURN Type_Tlog04_Stg_Header IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog04_Stg_Header(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Detail_Rec RETURN Type_Tlog04_Stg_Detail IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog04_Stg_Detail(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Tender_Rec RETURN Type_Tlog04_Stg_Tender IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog04_Stg_Tender(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Discount_Rec RETURN Type_Tlog04_Stg_Discount IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog04_Stg_Discount(NULL,
                                    NULL,
                                    NULL,
                                    NULL,
                                    NULL,
  ...
  ********************************************************************
  ********************************************************************/
  FUNCTION Initialize_Reward_Rec RETURN Type_Tlog04_Stg_Reward IS
    /* resets the array */
  BEGIN
    RETURN Type_Tlog04_Stg_Reward(NULL,
                                  NULL,
                                  NULL,
                                  NULL,
                                  NULL,
  ...
  EXCEPTION
    WHEN No_Data_Found THEN
      ...
      BEGIN
        ...
        INSERT INTO LOG_TXN_KEYS_stage4
          (Txnheaderid,
           Store_Nbr,
           Register_Nbr,
           Txn_Date,
  ...
  ********************************************************************/
  ...
  PROCEDURE Stage_unprocessed_Tlog04 (p_Dummy VARCHAR2,
    ...
    */
    ...
    v_Jobname          VARCHAR2(256) := 'UnProcessedTlog04';
  BEGIN
    /* data need to be written to stage tables */
    Log_Process_State('lw_txndetail_stage4');
...
/* get job log id for this process and for the dap process */
    ...
    select count(*) into v_Messagesreceived
      ...
      AND NOT EXISTS (SELECT 1
                      FROM lw_txndetail_stage4 x
                      WHERE x.rowkey = t.rowkey);


    INSERT INTO lw_txndetail_stage4
            (Rowkey
            ,Ipcode
            ,Vckey
            ,Dtlquantity
    ...
    Log_Process_State('lw_txntender_stage4');


    INSERT INTO lw_txntender_stage4
            (Rowkey
            ,Ipcode
            ,Vckey
            ,Processid
    ...
    COMMIT;
    /* data need to be written to hist tables */
    Log_Process_State('lw_txnrewardredeem_stage4');



    INSERT INTO lw_txnrewardredeem_stage4
            (Rowkey
            ,Ipcode
            ,Vckey
            ,Txnheaderid
      ...
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
                      FROM lw_txnrewardredeem_stage4 x
                      WHERE t.Rowkey = x.Rowkey)
      AND NOT EXISTS (SELECT 1
                      FROM Ats_Txnheader x
                      WHERE x.a_Txnheaderid = To_Char(t.Txnheaderid));
    ...
    COMMIT;
    /* data need to be written to hist tables */
    Log_Process_State('lw_txndetaildiscount_stage4');

    INSERT INTO lw_txndetaildiscount_stage4
            (Rowkey
            ,Ipcode
            ,Vckey
            ,Processid
      ...
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
                      FROM lw_txndetaildiscount_stage4 x
                      WHERE t.Rowkey = x.Rowkey)
      AND NOT EXISTS (SELECT 1
                      FROM Ats_Txnheader x
                      WHERE x.a_Txnheaderid = To_Char(t.Txnheaderid));
  ...
  END stage_unprocessed_Tlog04;
  /********************************************************************
  ********************************************************************
  ********************************************************************
  ********************************************************************/
  /******************* Internal function to saves unprocessed txns ***/
  PROCEDURE log_unprocessed_Tlog04 (p_JobNumber NUMBER DEFAULT -1) AS
    PRAGMA AUTONOMOUS_TRANSACTION; /* <---  allow to only commits activity in this local proc */
    /*
      this process will move unprocessed staged data to the unprocessed tables prior to truncating the staging tables.
      This should be called after each sucessful run of the tlog process from the hist_tlog proc, prior to it's actuall operations...
  ...
  BEGIN
    ...
    INSERT INTO Log_Txnrwdrdm_Unprocessed
      ...
      SELECT t.Rowkey
            ...
            ,t.Updatedate
            ,t.Lastdmlid
      FROM lw_txnrewardredeem_stage4 t
      WHERE 1=1
      AND t.processid = 0
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
    ...
    INSERT INTO Log_Txndtldisc_Unprocessed
      ...
      SELECT t.Rowkey
            ...
            ,t.Updatedate
            ,t.Lastdmlid
      FROM lw_txndetaildiscount_stage4 t
      WHERE 1=1
      AND t.processid = 0
      AND v_JobNumber IN (-1,t.fileid)
      AND NOT EXISTS (SELECT 1
  ...
  ********************************************************************/
  ...
  PROCEDURE Hist_Tlog04(p_Dummy VARCHAR2, p_Process_Date Date,
    ...
    */
    ...
    v_Dt               DATE;
    v_Jobstatus        NUMBER := 0;
    v_Jobname          VARCHAR2(256) := 'Hist_Tlog04';
    --log msg attributes
    v_Messageid  NUMBER;
    v_Envkey     VARCHAR2(256) := 'BP_AE@' ||
                                  Upper(Sys_Context('userenv',
                                                    'instance_name'));
    v_Logsource  VARCHAR2(256) := 'Hist_Tlog04';
    v_Batchid    VARCHAR2(256) := 0;
    v_Message    VARCHAR2(256);
    v_Reason     VARCHAR2(256);
    v_Error      VARCHAR2(256);
  ...
  BEGIN
   ...
   BEGIN
       ...
       ELSE
        ...
        FROM   LW_LIBJOB lj
        where  lj.jobname = 'DAP-Tlog04'
        AND    trunc(lj.starttime) = trunc(v_ProcessDate)
        AND    to_char(lj.starttime, 'mm/dd/yyyy hh:mm:ss') = (select to_char(max(starttime), 'mm/dd/yyyy hh:mm:ss') from lw_libjob where  jobname = 'DAP-Tlog04');
       END IF;
...
--AE0-1245: BEGIN Create DML error exception handling for post tlog processing
  WHEN Dml_Errors THEN
    ...
    LOOP
      ...
      v_Reason         := 'Failed Procedure Hist_Tlog04: DML_Error ';
      v_Message        := 'DML_error';
      Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                p_Envkey    => v_Envkey,
                p_Logsource => 'Hist_Tlog04',
                p_Filename  => 'Hist_Tlog04',
                p_Batchid   => v_Batchid,
                p_Jobnumber => v_My_Log_Id,
                p_Message   => v_Message,
                p_Reason    => v_Reason,
...
--AE0-1245: END Create DML error exception handling for post tlog processing
      WHEN No_Data_Found THEN
                      v_Messagesfailed := v_Messagesfailed + 1;
                      v_Error          := SQLERRM;
                      v_Reason         := 'DAP Tlog04 job is not finished';
                      v_Message        := 'DAP Tlog04 job is not finished';
      ...
      Raise_Application_Error(-20001, 'DAP Tlog04 job is not finished');
    ...
    THEN
                      v_Messagesfailed := v_Messagesfailed + 1;
                      v_Error          := SQLERRM;
                      v_Reason         := 'DAP Tlog04 did not process any records';
                      v_Message        := 'DAP Tlog04 did not process any records';
                      ...
                      Utility_Pkg.Log_Msg(p_Messageid => v_Messageid,
                                          ...
                                          p_Trycount  => v_Trycount,
                                          p_Msgtime   => SYSDATE);
      Raise_Application_Error(-20001, 'DAP Tlog04 did not process any records');
    END IF;
...
--AE0-1245: END Create DML error exception handling for post tlog processing
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG4</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog04.Update.Ats_Historytxndetail</proc>' || Chr(10) ||
      ...
      Raise_Application_Error(-20002, 'STAGE_TLOG4 job is not finished');
...
--AE0-1245: END Create DML error exception handling for post tlog processing
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG4</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog04.Insert.Ats_Historytxndetail</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
--AEO-771 END
    ...
    (select txnheaderid from lw_txndetail_stage4 where ipcode > 0 and ProcessId = 0 and NonMember = 0);

    -- Tlog Txn ErrorCouunt = ProcessCount ? SuccessCount
    v_Messagesfailed := v_Messagesreceived - v_Messagespassed;


    EXECUTE IMMEDIATE 'truncate table lw_txndetail_stage4';
...
--AE0-1245: END Create DML error exception handling for post tlog processing
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG4</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog04.Insert.Ats_Historytxntender</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
/* PI28864 changes end here --  SCJ */
    EXECUTE IMMEDIATE 'truncate table lw_txntender_stage4';
...
--AE0-1245: END Create DML error exception handling for post tlog processing
    WHEN OTHERS THEN
      ...
      v_Message := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                   '    <pkg>STAGE_TLOG4</pkg>' || Chr(10) ||
                   '    <proc>Hist_Tlog04.Insert.Ats_Historytxnrewardredeem</proc>' || Chr(10) ||
                   '  </details>' || Chr(10) || '</failed>';
      Utility_pkg.Log_msg(p_messageid => v_messageid,
                          p_envkey    => v_envkey,
                          p_logsource => v_logsource,
...
--AEO-771 END
/* PI28864 changes end here --  SCJ */
    EXECUTE IMMEDIATE 'truncate table lw_txnrewardredeem_stage4';
...
END;
    ...
    EXECUTE IMMEDIATE 'truncate table lw_txndetaildiscount_stage4';
--AEO-771 END
      /* PI28864 changes end here --  SCJ */
--AEO-771 BEGIN
BEGIN
...
END;
    Log_Process_State('LOG_TXN_KEYS');
    log_unprocessed_Tlog04();
--AEO-771 END

    EXECUTE IMMEDIATE 'truncate table LOG_TXN_KEYS_stage4';
    ...
    EXECUTE IMMEDIATE 'Truncate TABLE LW_txnheader_Wrk4' ; -- clear work table before next insert
    EXECUTE IMMEDIATE 'Truncate TABLE LW_txndetailitem_Wrk4' ; -- clear work table before next insert
...
--AEO-771 END
  ...
  ********************************************************************
  ********************************************************************/
  PROCEDURE Hist_Tlog04_ByJobNumber(p_Dummy VARCHAR2,
                                    p_JobNumber NUMBER,
                                    Retval  IN OUT Rcursor) AS
    /*
      this process will move staged data to the history tables then truncate the staging tables.
    ...
    */
    v_jobNumber  NUMBER := nvl(p_JobNumber,-1);
  BEGIN
    log_unprocessed_Tlog04 (p_JobNumber => p_JobNumber);
    /* data need to be written to hist tables */
    Log_Process_State('ATS_HISTORYTXNDETAIL');
    ...
    Delete From lw_txntender_stage4 t Where t.fileid = p_JobNumber;
    ...
    Delete From lw_txnrewardredeem_stage4 t Where t.fileid = p_JobNumber;
    ...
    Delete From lw_txndetaildiscount_stage4 t Where t.fileid = p_JobNumber;
    ...
    DELETE FROM LOG_TXN_KEYS_stage4 t
    WHERE t.txnheaderid IN (SELECT txn.txnheaderid FROM lw_txndetail_stage4 txn WHERE txn.fileid = p_JobNumber);

    Delete From lw_txndetail_stage4 t Where t.fileid = p_JobNumber;
    commit;

    --commented out this statement because txns can come in late
    --    EXECUTE IMMEDIATE 'truncate tablel LOG_TXN_COUNTS';

  END Hist_Tlog04_ByJobNumber;
  ...
  ********************************************************************/
  ...
  BEGIN
    ...
    COMMIT;
    /* empty out arrays */
    gv_Detail_Tbl     := Type_Tlog04_Stg_Detail_Tbl();
    gv_Tender_Tbl     := Type_Tlog04_Stg_Tender_Tbl();
    gv_Discount_Tbl   := Type_Tlog04_Stg_Discount_Tbl();
    gv_Reward_Tbl     := Type_Tlog04_Stg_Reward_Tbl();
  ...
  ********************************************************************/
  /******************* Internal function to Load  date ************/
  PROCEDURE Load_Stage_Data(p_Head_Rec       IN OUT Type_Tlog04_Stg_Header,
                            p_Detail_Tbl     IN Type_Tlog04_Stg_Detail_Tbl,
                            p_Tender_Tbl     IN Type_Tlog04_Stg_Tender_Tbl,
                            p_Discount_Tbl   IN Type_Tlog04_Stg_Discount_Tbl,
                            p_Reward_Tbl     IN Type_Tlog04_Stg_Reward_Tbl,
...
/*National Rollout Changes AEO-1218 here        ----------------SCJ*/
  ...
  /*National Rollout Changes AEO-1218 here        ----------------SCJ*/
    ...
    */
    IF p_Detail_Tbl.Count > 0 THEN
      FOR i IN 1..p_Detail_Tbl.count LOOP
        ...
        gv_Detail_Tbl(gv_Detail_Tbl.count) :=
                         Type_Tlog04_Stg_Detail(/* rowkey               */p_Detail_Tbl(i).Rowkey,
    ...
    ELSIF p_Detail_Tbl.Count = 0 AND v_StoreNumber = 659 AND
      ...
      gv_Detail_Tbl.extend;
      gv_Detail_Tbl(gv_Detail_Tbl.count) :=
                       Type_Tlog04_Stg_Detail(/* rowkey               */ Seq_Rowkey.Nextval,
                                              /* ipcode               */       p_Head_Rec.Ipcode,
                                              /* vckey                */       p_Head_Rec.Vckey,
                                              /* parentrowkey         */       NULL,
                                              /* dtlquantity          */       0,
    ...
    IF p_Discount_Tbl.Count > 0 THEN
      ...
      FOR i IN 1..p_Discount_Tbl.count LOOP
         ...
         gv_Discount_Tbl(gv_Discount_Tbl.count) :=
              Type_Tlog04_Stg_Discount    (/* rowkey         */p_Discount_Tbl(i).Rowkey,
                                           /* ipcode         */p_Head_Rec.Ipcode,
                                           /* vckey          */p_Head_Rec.Vckey,
                                           /* processid      */p_Head_Rec.Processid ,
    ...
    IF p_Reward_Tbl.Count > 0 THEN
      ...
      /* insert redeem records from previous txn */
        ...
        gv_Reward_Tbl(gv_Reward_Tbl.count) :=
                     Type_Tlog04_Stg_Reward( /*rowkey                    */p_Reward_Tbl(i).Rowkey,
                                             /*ipcode                    */p_Head_Rec.Ipcode,
                                             /*vckey                     */p_Head_Rec.Vckey,
                                             /*processid                 */p_Head_Rec.Processid,
    ...
    IF p_Tender_Tbl.Count > 0 THEN
      ...
      /* insert tenders from previous txn */
        ...
        gv_Tender_Tbl(gv_Tender_Tbl.count) :=
                     Type_Tlog04_Stg_Tender( /* rowkey         */p_Tender_Tbl(i).Rowkey,
                                             /* ipcode         */p_Head_Rec.Ipcode,
                                             /* vckey          */p_Head_Rec.Vckey,
                                             /* processid      */p_Head_Rec.Processid,
    ...
    ELSE
      ...
      gv_Tender_Tbl.extend;
      gv_Tender_Tbl(gv_Tender_Tbl.count) :=
                   Type_Tlog04_Stg_Tender( /* rowkey         */Seq_Rowkey.Nextval,
                                           /* ipcode         */p_Head_Rec.Ipcode,
                                           /* vckey          */p_Head_Rec.Vckey,
                                           /* processid      */p_Head_Rec.Processid,
                                           /* storeid        */p_Head_Rec.Txnstoreid,
  ...
  ********************************************************************/
  ...
  PROCEDURE Stage_Tlog04(p_Filename   VARCHAR2 DEFAULT NULL,
    ...
    */
    /* raw data records */
    v_00_Rec   Type_Tlog04_00header;
    v_01_Rec   Type_Tlog04_01item;
    v_02_Rec   Type_Tlog04_02subtotal;
    v_03_Rec   Type_Tlog04_03salestax;
    v_04_Rec   Type_Tlog04_04tender;
    v_05_Rec   Type_Tlog04_05giftcert;
    v_14_Rec   Type_Tlog04_14foundorder;
    v_15_Rec   Type_Tlog04_15ceditcrdcheck;
    v_18_Rec   Type_Tlog04_18txndiscount;
    v_2733_Rec Type_Tlog04_2733origtxn;
    v_2751_Rec Type_Tlog04_2751discountrsn;
    v_2762_Rec Type_Tlog04_2762valuecard;
    v_2777_Rec Type_Tlog04_2777multidivmsttxn;
    v_2792_Rec Type_Tlog04_2792pomogiftsku;
    v_2793_Rec Type_Tlog04_2793custloyalyt;
    v_2799_Rec Type_Tlog04_2799directordernbr;
    v_99_Rec   Type_Tlog04_99item;    /*AEO-820 changes here -----------------SCJ*/
    /* records and collections used to stage a single txn */
    v_Head_Rec     Type_Tlog04_Stg_Header;
    v_Detail_Rec   Type_Tlog04_Stg_Detail;
    v_Detail_Tbl   Type_Tlog04_Stg_Detail_Tbl := Type_Tlog04_Stg_Detail_Tbl();
    v_Tender_Rec   Type_Tlog04_Stg_Tender;
    v_Tender_Tbl   Type_Tlog04_Stg_Tender_Tbl := Type_Tlog04_Stg_Tender_Tbl();
    v_Discount_Rec Type_Tlog04_Stg_Discount := Type_Tlog04_Stg_Discount(NULL,
                                                                        NULL,
                                                                        NULL,
                                                                        NULL,
                                                                        NULL,
                                                                        ...
                                                                        NULL,
                                                                        NULL);
    v_Discount_Tbl Type_Tlog04_Stg_Discount_Tbl := Type_Tlog04_Stg_Discount_Tbl();
    v_Reward_Rec   Type_Tlog04_Stg_Reward;
    v_Reward_Tbl   Type_Tlog04_Stg_Reward_Tbl := Type_Tlog04_Stg_Reward_Tbl();
    /* misc variables */
    ...
    v_Dt               DATE;
    v_Jobstatus        NUMBER := 0;
    v_Jobname          VARCHAR2(256) := 'Tlog04';
    --log msg attributes
    v_Messageid  NUMBER;
    v_Envkey     VARCHAR2(256) := 'BP_AE@' ||
                                  Upper(Sys_Context('userenv',
                                                    'instance_name'));
    v_Logsource  VARCHAR2(256) := 'Tlog04';
    v_Batchid    VARCHAR2(256) := 0;
  ...
  BEGIN
   ...
   /* ELSIF v_N1 > 0
    ...
    -- Log_process_state('S:hist_Tlog04');
      ...
      THEN
        ...
        THEN
          ...
          */
          ...
          v_00_Rec := Get_Tlog04_00header(p_Rec => c.Rec);
        ...
        THEN
          ...
          THEN
            ...
            v_Detail_Tbl          := Type_Tlog04_Stg_Detail_Tbl();
            v_Head_Rec            := Initialize_Head_Rec();
            v_Tender_Tbl          := Type_Tlog04_Stg_Tender_Tbl();
            v_Discount_Tbl        := Type_Tlog04_Stg_Discount_Tbl();
            v_Reward_Tbl          := Type_Tlog04_Stg_Reward_Tbl();
          ...
          THEN
            ...
            v_01_Rec := Get_Tlog04_01item(c.Rec);
...
*/
          ...
          THEN
            /* load rawdata to record type */
            v_99_Rec := Get_Tlog04_99item(c.Rec);
...
*/
          ...
          IF c.Rec_Type = '02'
          THEN
            v_02_Rec             := Get_Tlog04_02subtotal(c.Rec);
            v_Head_Rec.Txnamount := v_02_Rec.Amount;
          END IF;
          /* sales tax  record */
          IF c.Rec_Type = '03'
          THEN
            v_03_Rec                := Get_Tlog04_03salestax(c.Rec);
          ...
          THEN
            ...
            */
            ...
            v_04_Rec := Get_Tlog04_04tender(c.Rec);
          ...
          THEN
            ...
            v_05_Rec := Get_Tlog04_05giftcert(c.Rec);
          ...
          IF c.Rec_Type = '14'
          THEN
            v_14_Rec := Get_Tlog04_14foundorder(c.Rec);
...
/* redesign changes  end here --------------------------------------------------  SCJ  */
          ...
          THEN
            ...
            --v_15_rec :=get_Tlog04_15ceditcrdcheck (c.rec);
          END IF;
          /* txn discount string   record */
          IF c.Rec_Type = '18'
          THEN
            /* load raw data to record type */
            v_18_Rec := Get_Tlog04_18txndiscount(c.Rec);
          ...
          THEN
            /* load raw data to record type */
            v_2733_Rec := Get_Tlog04_2733origtxn(c.Rec);
          ...
          THEN
            /* load raw data to record type */
            v_2751_Rec := Get_Tlog04_2751discountrsn(c.Rec);
...
*/
          ...
          THEN
            /* load raw data to record type */
            v_2762_Rec := Get_Tlog04_2762valuecard(c.Rec);
            v_Detail_Rec              := Initialize_Detail_Rec();
          ...
          THEN
            ...
            v_2777_Rec := Get_Tlog04_2777multidivmsttxn(c.Rec);
            /* load alt info */
          ...
          /*    dont need this, not on tlog spec doc
                  IF c.rec_type = '27' AND c.user_id ='92' THEN
                    v_2792_rec :=get_Tlog04_2792pomogiftsku (c.rec);
                    --do something here
                  END IF;
          */
          /* Loyalty ID   record */
          ...
          THEN
            /* load raw data to record type */
            v_2793_Rec              := Get_Tlog04_2793custloyalyt(p_Rec => c.Rec);
          ...
          THEN
            /* load raw data to record type */
            v_2799_Rec := Get_Tlog04_2799directordernbr(c.Rec);
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      Utility_Pkg.Log_Job(p_Job              => v_My_Log_Id,
                          ...
                          p_Messagesfailed   => v_Messagesfailed,
                          p_Jobstatus        => v_Jobstatus,
                          p_Jobname          => 'stage4' || v_Jobname);
      v_Messagesfailed := v_Messagesfailed + 1;
      v_Error          := SQLERRM;
      v_Reason         := 'Failed Procedure Stage_Tlog04';
      v_Message        := '<failed>' || Chr(10) || '  <details>' || Chr(10) ||
                          '    <pkg>Stage_Tlog4</pkg>' || Chr(10) ||
                          '    <proc>Stage_Tlog04</proc>' || Chr(10) ||
                          '    <filename>' || v_Filename || '</filename>' ||
